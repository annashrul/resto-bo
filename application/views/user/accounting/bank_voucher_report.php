					...
					<div class="col-xs-12">
						<div class="box-body table-responsive">
							<table id="example1" class="table table-bordered table-striped dataTable">
								<thead>
									<tr>
										<th>No</th><th>Action</th><th>Bank Voucher No.</th><th>Date</th><th>Account</th><th>Descrip</th>
										<th>Currency</th><th>Exchange</th><th>Receiver</th><th>User</th><th>Total</th>
									</tr>
								</thead>
								<tbody>
								<?php $no = 0; $total = 0;
								foreach($bank_voucher_report as $row){ $no++; ?>
									<tr>
										<td><?=$no?></td>
										<td>
											<div class="btn-group">
												...
												<ul class="dropdown-menu" role="menu">
													...
													<!--<li class="divider"></li>-->
													<li><div class="col-sm-12"><?=anchor($content.'/print/?trx='.$row['id_bank_voucher'], '<i class="fa fa-print"></i> Print', array('class'=>'btn btn-default col-sm-12'))?></div></li>
													<!--<button class="btn btn-danger" onclick="hapus('coa', 'coa_id', '<?=$row['coa_id']?>')"><i class="fa fa-trash"></i> Delete</button>-->
												</ul>
											</div>
										</td>
										<td><?=$row['id_bank_voucher']?></td>
										<td><?=$row['tanggal']?></td>
...
<?php $i = 0; $total = 0;
foreach($bank_voucher_report as $row){ $i++; ?>
<!-- Modal-->
					...
					<div class="col-sm-12">
						<div class="col-sm-4">
							<div class="small-box bg-blue">
								<div class="inner">
									<label class="control-label text-left">Bank Voucher No.</label>
									<input class="form-control" readonly value="<?=$row['id_bank_voucher']?>"/>
								</div>
							</div>
						</div>
						<div class="col-sm-4">
						...
						<div class="col-xs-12">
							<div class="box-body table-responsive">
								<table id="example1" class="table table-bordered table-striped dataTable">
									...
									<tbody>
									<?php $no = 0; $debit = 0;
									$detail = $this->m_crud->read_data('acc_general_journal', '*', "id_trx = '".$row['id_bank_voucher']."' and coa <> '".$row['coa']."'", 'tanggal desc, id_general_journal asc');
