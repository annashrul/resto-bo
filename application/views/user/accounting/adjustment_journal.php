    ...
    <div class="box">
        <div class="box-header with-border">
            ...
            <div class="box-tools pull-right">
				<?php if (substr($access->access,23,1) == 1 ){ ?><button type="button" data-toggle="modal" data-target="#trx" class="btn btn-primary">Transaction Report</button><?php } ?>
				...
				<div class="col-sm-12">
					<div class="col-sm-7">
						<div class="col-sm-12" style="margin-bottom: 5px;">
							<div class="col-sm-4"><label>Adjustment Journal No.</label></div>
							<div class="col-sm-5">
								<?php (isset($_POST['tgl_quo'])&&$_POST['tgl_quo']!=null)? $tgl = substr($_POST['tgl_quo'], 2, 2).substr($_POST['tgl_quo'], 5, 2): $tgl = date('ym') ?>
								<?php $trx = $this->m_website->lokasi(null,'serial')."-ADJJ-".$tgl."-";
								$seri = (int) $this->m_crud->get_data($table, "max(substring(id_trx, ".(strlen($trx)+1).", 3)) as id", "id_trx like '".$trx."%'")['id'];
								$seri++; $seri = str_pad($seri, 3, '0', STR_PAD_LEFT); ?>
								<input class="form-control" type="text" name="id_aj" id="id_aj" value="<?=$trx.$seri?>" readonly /> 
								<?=form_error('id_aj', '<div class="error" style="color:red;">', '</div>')?>  
							</div>
						</div>
						<div class="col-sm-12" style="margin-bottom: 5px;">
							...
							<div class="col-sm-5">
								<div class="input-group date">
									...
									<input class="form-control datepicker_back pull-right" readonly onchange="trx_number()"
					...
					<div class="col-sm-5">
						<div class="col-sm-12" style="margin-bottom: 5px;">
							...
							<div class="col-sm-8">
								...
								echo form_dropdown('currency', $option, set_value('currency'), array('class' => 'form-control select2', 'id'=>'currency', 'onchange'=>'kurs_rate()', 'required'=>'required')); ?>
						...
						<div class="col-sm-12" style="margin-bottom: 5px;">
							...
							<div class="col-sm-8">
								<input class="form-control" onchange="hitungDiskon()" onkeyup="hitungDiskon()" onclick="hitungDiskon()" type="number" step="any" name="exchange" id="exchange" value="<?=set_value('exchange')?>" required />
						...
						<div class="col-xs-12">
							<div class="box-body table-responsive">
								<table id="example" class="table table-bordered table-striped dataTable">
									<thead>
										<tr>
											<th>No</th><th>Journal Entry No.</th><th>Date</th><th>Total</th><th>Action</th>
										</tr>
									</thead>
									<tbody>
										...
										$i = 0; //mysqli $transaction = $this->m_crud->read_data($table, '*, sum(debit) as total, group_concat(descrip SEPARATOR " ") AS description', null, 'tanggal desc', 'id_trx');
										$transaction = $this->m_crud->read_data($table, "id_trx, tanggal, sum(debit) as total", $where, 'tanggal desc', 'id_trx, tanggal');
										foreach($transaction as $row){ $i++; ?>
											<tr>
												<td><?=$i?></td>
												<td><?=$row['id_trx']?></td>
												<td><?=$row['tanggal']?></td>
												<td><?=$this->cart->format_number($row['total'])?></td>
												<td><?=anchor($content.'_report/print/?trx='.$row['id_trx'], '<i class="fa fa-print"></i> Print', array('class'=>'btn btn-primary', 'target'=>'_blank'))?></td>
											</tr>
										<?php } ?>
									</tbody>
								</table>
...
function trx_number(){
	var table = '<?=$table?>';
	var column = 'id_trx';
	var trx = '<?=$this->m_website->lokasi(null,'serial')?>-ADJJ';
	...
	$.ajax({
		...
		//data: {delete_id : id},
		success: function (data) { $('#id_aj').val(data); },
