    ...
    <div class="box">
        <div class="box-header with-border">
            ...
            <div class="box-tools pull-right">
				<?php if (substr($access->access,88,1) == 1 ){ ?><button type="button" data-toggle="modal" data-target="#trx" class="btn btn-primary">Transaction Report</button><?php } ?>
				...
				<div class="col-sm-12">
					<div class="col-sm-7">
						<div class="col-sm-12" style="margin-bottom: 5px;">
							<div class="col-sm-4"><label>Tico Voucher No.</label></div>
							<div class="col-sm-6">
								<input class="form-control" type="text" name="id_tv" id="id_tv" value="<?=set_value('id_tv')?>" readonly />
								<?=form_error('id_tv', '<div class="error" style="color:red;">', '</div>')?>
							</div>
						</div>
						<div class="col-sm-12" style="margin-bottom: 5px;">
						...
						<div class="col-sm-12" style="margin-bottom: 5px;">
							...
							<div class="col-sm-6">
								<div class="input-group">
									...
									$coa = $this->m_crud->read_data('coa', '*', 'coa_id like "11%" and (nama like "%TICO%" or coa_id = "11102" or coa_id = "11103")', 'coa_id asc');
					...
					<div class="col-xs-12">
						<div class="box-body table-responsive">
							<table id="table_transaksi" class="table table-bordered table-striped dataTable">
								...
								<tbody>
									<?php $option = null; $option[''] = '-- Select --';
									$coa = $this->m_crud->read_data('coa', '*', 'nama like "%TICO%"', 'coa_id asc');
						...
						<div class="col-xs-12">
							<div class="box-body table-responsive">
								<table id="table_transaksi" class="table table-bordered table-striped dataTable">
									<thead>
										<tr>
											<th>No</th><th>Tico Voucher No.</th><th>Date</th><th>Account</th><th>Description</th><th>Total</th><th>Action</th>
										</tr>
									</thead>
									<tbody>
										<?php $i = 0; $transaction = $this->m_crud->read_data($table, '*, (select sum(debit) from acc_general_journal where id_trx = id_tico_voucher) as total', null, 'tanggal desc');
...
<script>
/*function no_trx(){
	var trx = <?=json_encode($this->m_accounting->trx_tico_number())?>;
	for(var i=0; i<trx.length; i++){
		if(trx[i]['coa']==$('#account').val()){ 
			$('#id_tv').val(trx[i]['trx']);
		}
	}
}*/
...
function trx_number(){
	...
	$.ajax({
		...
		//data: {delete_id : id},
		success: function (data) { $('#id_tv').val(data); },
