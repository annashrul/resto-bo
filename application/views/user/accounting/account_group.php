        ...
        <div class="box-body">
			...
			<div class="row" style="margin-bottom: 10px;">
				<div class="col-sm-12">
					...
					<?php //anchor($content.'/add', '<i class="fa fa-plus"></i> Add', array('class'=>'pull-right btn btn-primary'))?>
				</div>
			</div>
			<?= form_close(); ?>
			<div class="box">
					...
					<div class="col-xs-12">
						<div class="box-body table-responsive">
							<table id="example1" class="table table-bordered table-striped dataTable">
								<thead>
									<tr>
										<th>No</th><!--<th>Action</th>--><th>Account Category</th><th>Account Group</th><th>Name</th>
									</tr>
								</thead>
								<tbody>
								<?php $no = 0; foreach($coa_group as $row){ $no++; ?>
									<tr>
										<td><?=$no?></td>
										<!--<td>
											<div class="btn-group">
												...
												<ul class="dropdown-menu" role="menu">
													<li><div class="col-sm-12"><?=anchor($content.'/edit/?trx='.$row['group_id'], '<i class="fa fa-edit"></i> Edit', array('class'=>'btn btn-default col-sm-12'))?></div></li>
													<!--<li class="divider"></li>--
													<li><div class="col-sm-12"><?=anchor($content.'/delete/?trx='.$row['group_id'], '<i class="fa fa-trash"></i> Delete', array('class'=>'btn btn-default col-sm-12', 'onclick'=>"return confirm('Delete Data?');"))?></div></li>
													<!--<button class="btn btn-danger" onclick="hapus('coa', 'coa_id', '<?=$row['coa_id']?>')"><i class="fa fa-trash"></i> Delete</button>--
												</ul>
											</div>
										</td>-->
										<td><?=$row['kat_id'].' - '.$row['kat_nama']?></td>
										<td><?=$row['group_id']?></td>
