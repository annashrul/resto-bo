					...
					<div class="col-xs-12">
						<div class="box-body table-responsive">
							<table id="example1" class="table table-bordered table-striped dataTable">
								<thead>
									<tr>
										<th>No</th><th>Adjustment Journal No.</th><th>Date</th><th>Currency</th>
										<th>Exchange</th><th>User</th><th>Total</th><th>Action</th>
									</tr>
								</thead>
								<tbody>
								<?php $no = 0; $total = 0;
								foreach($adjustment_journal_report as $row){ $no++; ?>
									<tr>
										<td><?=$no?></td>
										<td><?=$row['id_trx']?></td>
										<td><?=$row['tanggal']?></td>
										...
										<td style="text-align:right;"><?=$this->cart->format_number($row['total'])?></td>
										<td>
											<button type="button" data-toggle="modal" data-target="#<?=$no?>" class="btn btn-primary"><i class="fa fa-eye"></i> Detail</button>
											<?=anchor($content.'/print/?trx='.$row['id_trx'], '<i class="fa fa-print"></i> Print', array('class'=>'btn btn-primary'))?>
										</td>
									</tr>
...
<?php $i = 0; $total = 0;
foreach($adjustment_journal_report as $row){ $i++; ?>
<!-- Modal-->
					...
					<div class="col-sm-12">
						<div class="col-sm-4">
							<div class="small-box bg-blue">
								<div class="inner">
									<label class="control-label text-left">Adjustment Journal No.</label>
									<input class="form-control" readonly value="<?=$row['id_trx']?>"/>
								</div>
							</div>
						</div>
						<div class="col-sm-4">
						...
						<div class="col-xs-12">
							<div class="box-body table-responsive">
								<table id="example1" class="table table-bordered table-striped dataTable">
									<thead>
										<tr>
											<th>No</th><th>Description</th><th>Code</th><th>Account Name</th><th>Debit</th><th>Credit</th>
										</tr>
									</thead>
									<tbody>
									<?php $no = 0; $debit = 0; $credit = 0; 
									$detail = $this->m_crud->read_data('acc_adjustment_journal', '*', "id_trx = '".$row['id_trx']."'", 'tanggal desc, id_adjustment_journal asc');
									foreach($detail as $rows){ $no++; ?>
										<tr>
											<td><?=$no?></td>
											<td><?=$rows['descrip']?></td>
											<td><?=$rows['coa']?></td>
											<td><?php $caption = $this->m_crud->get_data('coa', '*', "coa_id = '".$rows['coa']."'"); echo $caption['nama'] ?></td>
											<td style="text-align:right;"><?=$this->cart->format_number($rows['debit']/$rows['rate'])?></td>
											<td style="text-align:right;"><?=$this->cart->format_number($rows['credit']/$rows['rate'])?></td>
										</tr>
