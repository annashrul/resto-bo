    ...
    <div class="box">
        <div class="box-header with-border">
            ...
            <div class="box-tools pull-right">
				<?php if (substr($access->access,21,1) == 1 ){ ?><button type="button" data-toggle="modal" data-target="#trx" class="btn btn-primary">Transaction Report</button><?php } ?>
				...
				<div class="col-sm-12">
					<div class="col-sm-7">
						<div class="col-sm-12" style="margin-bottom: 5px;">
							<div class="col-sm-4"><label>Journal Entry No.</label></div>
							<div class="col-sm-5">
								<?php (isset($_POST['tgl_quo'])&&$_POST['tgl_quo']!=null)? $tgl = substr($_POST['tgl_quo'], 2, 2).substr($_POST['tgl_quo'], 5, 2): $tgl = date('ym') ?>
								<?php $trx = $this->m_website->lokasi(null,'serial')."-JETR-".$tgl."-";
								$seri = (int) $this->m_crud->get_data($table, "max(substring(id_journal_entry, ".(strlen($trx)+1).", 3)) as id", "id_journal_entry like '".$trx."%'")['id'];
								$seri++; $seri = str_pad($seri, 3, '0', STR_PAD_LEFT); ?>
								<input class="form-control" type="text" name="id_je" id="id_je" value="<?=$trx.$seri?>" readonly />
								<?=form_error('id_je', '<div class="error" style="color:red;">', '</div>')?>
							</div>
						</div>
						<div class="col-sm-12" style="margin-bottom: 5px;">
								...
								<tbody>
									...
									<?php if($this->cart->contents()){ ?>
										...
										<tr>
											...
											<td><input onkeyup="total_balance()" onclick="total_balance()" class="form-control" type="text" placeholder="Debit" name="debit<?=$i?>" id="debit<?=$i?>" value="<?=$items['debit']?>"/></td>
											<td><input onkeyup="total_balance()" onclick="total_balance()" class="form-control" type="text" placeholder="Credit" name="credit<?=$i?>" id="credit<?=$i?>" value="<?=$items['credit']?>"/></td>
						...
						<div class="col-xs-12">
							<div class="box-body table-responsive">
								<table id="example" class="table table-bordered table-striped dataTable">
									<thead>
										<tr>
											<th>No</th><th>Journal Entry No.</th><th>Date</th><th>Description</th><th>Total</th><th>Action</th>
										</tr>
									</thead>
									<tbody>
										...
										$i = 0; $transaction = $this->m_crud->read_data($table, '*, (select sum(debit) from acc_general_journal where id_trx = id_journal_entry) as total', $where, 'tanggal desc');
...
function trx_number(){
	var table = '<?=$table?>';
	var column = 'id_journal_entry';
	var trx = '<?=$this->m_website->lokasi(null,'serial')?>-JETR';
	...
	$.ajax({
		...
		//data: {delete_id : id},
		success: function (data) { $('#id_je').val(data); },
