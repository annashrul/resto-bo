					...
					<div class="col-xs-12">
						<div class="box-body table-responsive">
							<table id="example1" class="table table-bordered table-striped dataTable">
								<thead>
									<tr>
										<th>No</th><th>Tico Voucher No.</th><th>Date</th><th>Account</th><th>Descrip</th>
										<th>Currency</th><th>Exchange</th><th>Receiver</th><th>User</th><th>Total</th><th>Action</th>
									</tr>
								</thead>
								<tbody>
								<?php $no = 0; $total = 0;
								foreach($tico_voucher_report as $row){ $no++; ?>
									<tr>
										<td><?=$no?></td>
										<td><?=$row['id_tico_voucher']?></td>
										<td><?=$row['tanggal']?></td>
										<td><?=$row['coa'].'-'.$this->m_accounting->coa($row['coa'], 'nama')?></td>
										<td><?=$row['descrip']?></td>
										<td><?=$this->m_gracia->kurs($row['currency'], 'nama')?></td>
										<td><?=$this->cart->format_number($row['rate'])?></td>
										<td><?=$row['penerima']?></td>
										<td><?=$this->m_gracia->user_data($row['user_id'])->nama?></td>
										...
										<td>
											...
											<?=anchor($content.'/print/?trx='.$row['id_tico_voucher'], '<i class="fa fa-print"></i> Print', array('class'=>'btn btn-primary'))?>
										</td>
									</tr>
...
<?php $i = 0; $total = 0;
foreach($tico_voucher_report as $row){ $i++; ?>
<!-- Modal-->
					...
					<div class="col-sm-12">
						<div class="col-sm-4">
							<div class="small-box bg-blue">
								<div class="inner">
									<label class="control-label text-left">Tico Voucher No.</label>
									<input class="form-control" readonly value="<?=$row['id_tico_voucher']?>"/>
								</div>
							</div>
						</div>
						<div class="col-sm-4">
						...
						<div class="col-sm-4">
							<div class="small-box bg-blue">
								<div class="inner">
									...
									<input class="form-control" readonly value="<?=$this->m_gracia->user_data($row['user_id'])->nama?>"/>
								</div>
							</div>
						</div>
						<div class="col-sm-4">
						...
						<div class="col-xs-12">
							<div class="box-body table-responsive">
								<table id="example1" class="table table-bordered table-striped dataTable">
									...
									<tbody>
									<?php $no = 0; $debit = 0;
									$detail = $this->m_crud->read_data('acc_general_journal', '*', 'id_trx = "'.$row['id_tico_voucher'].'" and coa <> "'.$row['coa'].'"', 'tanggal desc, id_general_journal asc');
