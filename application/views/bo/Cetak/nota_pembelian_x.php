...
<table width="100%" border="1">
    ...
    foreach ($list as $row){
        ...
        $qty = $qty + ((int)$row['jumlah_beli']-(int)$row['jumlah_retur']);
        if (ltrim(rtrim($row['barcode']))==ltrim(rtrim($row['kode_barang']))) {
            $brcd_art = $row['Deskripsi'];
        } else {
            $brcd_art = $row['barcode'];
        }
        echo '<tr><td class="isi center">'.$i.'</td><td class="isi">'.$row['kode_barang'].'</td><td class="isi">'.$brcd_art.'</td><td class="isi">'.$row['nm_brg'].'</td><td class="isi">'.$row['satuan'].'</td><td class="isi kanan">'.number_format($row['harga_beli'],2).'</td><td class="isi kanan">'.number_format($row['harga_jual'],2).'</td><td class="isi center">'.(($row['harga_jual']!=0)?round((1 - (($row['harga_beli']-($row['harga_beli']*($row['disc1']/100)))/$row['harga_jual']))*100, 2):'0').'</td><td class="isi center">'.($row['disc1']+0).'</td><td class="isi center">'.($row['ppn']+0).'</td><td class="isi center">'.((int)$row['jumlah_beli']-(int)$row['jumlah_retur']).'</td><td class="isi kanan">'.number_format($hitung_sub_total,2).'</td></tr>';
        $i++;
    }
