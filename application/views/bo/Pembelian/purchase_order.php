...
<script>
	...
	function get_tmp_detail(param = 0) {
		$.ajax({
			url: "<?php echo base_url() . 'pembelian/get_tr_temp_d_po' ?>",
			type: "GET",
			dataType: "JSON",
			success: function (data) {
	...
	function simpan_transaksi() {
        ...
        if (validate()) {
            $.ajax({
                url: "<?php echo base_url().'pembelian/trans_po_x' ?>",
                type: "POST",
				...
				success: function (data) {
                    if (data.status) {
						...
						if (confirm('Transaksi Berhasil! Akan mencetak transaksi?')){
                            cetak_transaksi('purchase_order_report', 'none', data.kode);
							//window.open("<?php echo base_url() . 'pembelian/purchase_order_report/print/' ?>"+data,'_blank');
						}
                    } else {
