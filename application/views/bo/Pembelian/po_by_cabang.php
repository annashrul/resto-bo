									...
									<div class="panel-body">
										...
										<div class="row">
											<div class="col-sm-6">
												...
												<input type="hidden" id="nota_po" name="nota_po" class="form-control" readonly value="">
												...
												<input readonly name="tgl_kirim" id="tgl_kirim" type="hidden" value="<?=set_value('tgl_kirim')?set_value('tgl_kirim'):date("Y-m-d")?>">
														
												<div class="row" style="margin-bottom: 3px">
													<label class="col-sm-4">Lokasi</label>
													<div class="col-sm-6">
														<select name="lokasi" id="lokasi" onclick="add_barang_po_by_cabang();" onchange="trx_number(); add_tmp_master(); hide_notif('alr_lokasi')" class="select2">
											...
											<div class="col-sm-6">
												<input type="hidden" name="jenis_transaksi" id="jenis_transaksi" value="Tunai" />
												<!--<div class="row" style="margin-bottom: 3px">
												...
												<div class="row" style="margin-bottom: 3px">
													...
													<div class="col-sm-6">
														<select type="text" name="supplier" id="supplier" onclick="add_barang_po_by_cabang();" onchange="add_tmp_master(); hide_notif('alr_supplier')" class="select2">
															<option value="">Pilih</option>
															<?php
...
<script>
	...
	function get_tmp_master() {
		$.ajax({
			...
			success: function (data) {
				...
				$('#tgl_order').datepicker("setDate", '<?=date('Y-m-d')?>');
	...
	function get_tmp_detail(param = 0) {
		...
		$.ajax({
			url: "<?php echo base_url() . 'pembelian/get_tr_temp_d_po_by_cabang' ?>",
			type: "POST",
			data:{lokasi:lokasi},
			dataType: "JSON",
			beforeSend: function () {
	...
	function add_barang_po_by_cabang(){
		var lokasi = $('#lokasi').val();
		var supplier = $('#supplier').val();
		
		if(lokasi!='' && supplier!=''){
			$.ajax({
				url: "<?php echo base_url() . 'pembelian/add_barang_po_by_cabang' ?>",
				type: "POST",
				data:{lokasi:lokasi, group1:supplier},
				dataType: "JSON",
		   		beforeSend: function () {
                    $('#loading').show();
                },
	...
	function qty_max(param=''){
		var valid = 1;
		
		for(var i=1; i<=$('#col').val(); i++){
			var qty = parseInt($('#d9'+i).val());
			var max = parseInt($('#stock_max'+i).text());
			if(qty > max){
				valid = 0;
				$('#alr_qty'+i).text('Qty melebihi max');
			} else {
				$('#alr_qty'+i).text('');
			}
		}
		
		if(param=='simpan'){ return valid; }
	}
	...
	function simpan_transaksi() {
        ...
        if (validate()) {
            $.ajax({
                url: "<?php echo base_url().'pembelian/trans_po_by_cabang' ?>",
                type: "POST",
                    ...
                    if (data != 0) {
						if('<?=(isset($_GET['trx'])?$_GET['trx']:'')?>'==''){
							if (confirm('Transaksi Berhasil! Akan mencetak transaksi?')){ 
								...
								cetak_transaksi('po_by_cabang_report', 'none', data);
							...
							window.location = "<?=base_url().'pembelian/po_by_cabang_report'?>";
						}		
                    } else {
