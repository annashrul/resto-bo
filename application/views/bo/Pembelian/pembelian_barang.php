										...
										<div class="row">
											...
											<div class="col-sm-6">
												...
												<div class="row" style="margin-bottom: 3px">
													...
													<div class="col-sm-6">
														<div class="input-group date">
															...
															<input class="form-control pull-right datepicker_back" readonly onchange="trx_number();add_tmp_master()" name="tgl_pembelian" id="tgl_pembelian" type="text" value="<?=set_value('tgl_pembelian')?set_value('tgl_pembelian'):date("Y-m-d")?>">
												...
												<div class="row" style="margin-bottom: 3px">
													...
													<div class="col-sm-6">
														<select name="lokasi_beli" id="lokasi_beli" onchange="trx_number(); hide_notif('alr_lokasi_beli')" class="select2">
											...
											<div class="col-sm-6">
                                                <div class="row" style="margin-bottom: 3px">
                                                    ...
                                                    <div class="col-sm-6">
                                                        <input type="text" id="nama_penerima" name="nama_penerima" onkeyup="hide_notif('alr_nama_penerima')" onblur="add_tmp_master()" class="form-control">
                                                        <b class="error" id="alr_nama_penerima"></b>
                                                    </div>
                                                </div>
												<div class="row" style="margin-bottom: 3px">
								...
								<div class="col-md-5">
									<div class="pull-right">
										...
										<div class="row" style="margin-bottom: 3px">
											...
											<div class="col-sm-3">
												<input onblur="update_tmp_master('m11', $(this).val())" onkeyup="hitung_total()" onclick="this.select()" type="number" id="pajak" name="pajak" class="form-control" placeholder="%">
											</div>
										</div>
										<div class="row" style="margin-bottom: 3px">
...
<script>
	...
	function cek_nota(no_nota) {
		if (no_nota != '') {
			$.ajax({
				url: "<?php echo base_url() . 'pembelian/check_nota/' ?>" + btoa(no_nota),
				type: "GET",
	...
	function get_tmp_master() {
		$.ajax({
			url: "<?php echo base_url() . 'pembelian/get_tr_temp_m' ?>",
			type: "GET",
			dataType: "JSON",
			success: function (data) {
				$("#nota_sistem").val(data['m1']);
				$('#tgl_pembelian').datepicker("setDate", data['m2']);
				$("#lokasi_beli").val(data['m3']).change();
				$("#jenis_transaksi").val(data['m4']).change();
				$("#supplier").val(data['m5']).change();
				$("#nota_supplier").val(data['m6']);
				$("#nama_penerima").val(data['m14']);
				$("#no_po").val(data['m7']).change();
				$("#tgl_jatuh_tempo").datepicker("setDate", data['m8']);
				cek_nota(data['m6']);
				if (data['m7'] != '-') {
					$("#no_po").select2('readonly',true);
    ...
    function add_tmp_master() {
        ...
        if (nota_sistem != '') {
            $.ajax({
                url: "<?php echo base_url() . 'pembelian/add_tr_temp_m' ?>",
                data: data,
                type: "POST",
	...
	function get_tmp_detail(param = 0, col = 0) {
		$.ajax({
			url: "<?php echo base_url() . 'pembelian/get_tr_temp_d/' ?>" + btoa('add'),
			type: "GET",
			dataType: "JSON",
			success: function (data) {
    ...
    function update_tmp_detail(barcode, column, value) {
        $.ajax({
            url: "<?php echo base_url() . 'pembelian/update_tr_temp_d/' ?>" + btoa(barcode) + "/" + btoa(column) + "/" + btoa(hapuskoma(value)),
            type: "GET"
        });
    }
	...
	function hapus_barang(barcode) {
		$.ajax({
			url: "<?php echo base_url() . 'pembelian/delete_tr_temp_d/' ?>" + btoa(barcode),
			type: "GET",
			success: function (data) {
				if (data) {
					get_tmp_detail();
	...
	function get_no_po(supplier) {
		$.ajax({
			url: "<?php echo base_url() . 'pembelian/get_no_po/' ?>" + btoa(supplier),
			type: "GET",
			success: function (data) {
	...
	function add_po() {
		...
		if (nota_sistem != '') {
			...
			$.ajax({
				url: "<?php echo base_url() . 'pembelian/add_po_list_barang/' ?>" + btoa(nota_sistem) + '/' + btoa(no_po),
				type: "GET",
				dataType: "JSON",
				success: function (data) {
					if (data) {
						$("#jenis_transaksi").val(data.po['jenis']).change();
						get_tmp_detail();
						if (no_po != '-') {
							$("#no_po").select2('readonly',true);
							$("#cari_no_po").prop("disabled", true);
    ...
    function add_barang() {
        ...
        $.ajax({
            ...
            data: {nota_sistem_: nota_sistem, list_: list},
            type: "POST",
            success: function (data) {
                if (data) {
                    get_tmp_detail();
	...
	$("#barcode").keyup(function (event) {
		...
		if (event.keyCode == 13) {
			...
			if (lokasi_beli != '' && supplier != '' && barcode != '') {
				$.ajax({
					url: "<?php echo base_url() . 'pembelian/get_barang/' ?>" + btoa(nota_sistem) + "/" + btoa(barcode) + "/" + btoa(lokasi_beli) + "/" + btoa(supplier) + "/" + btoa(cat_cari),
					type: "GET",
					dataType: "JSON",
					success: function (data) {
						if (data.status == 1) {
	...
	function batal_transaksi() {
        $.ajax({
            url: "<?php echo base_url().'pembelian/delete_trans_pembelian' ?>",
            type: "GET",
            success: function (data) {
                if (data) {
                    location.reload();
