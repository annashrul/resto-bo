							...
							<div class="row">
								<div class="col-md-12">
									<div class="panel-body">
										...
										<div class="row">
											<input type="hidden" name="param" value="edit_retur">
											<div class="col-sm-6">
...
<script>
	...
	function get_tmp_master() {
		$.ajax({
			url: "<?php echo base_url() . 'pembelian/get_tr_temp_m_retur/' ?>" + btoa('edit_retur'),
			type: "GET",
			dataType: "JSON",
    ...
    function update_tmp_master(column, data) {
        $.ajax({
            url: "<?php echo base_url() . 'pembelian/update_tr_temp_m/' ?>" + btoa(column) + "/" + btoa(data) + "/" + btoa('edit_retur'),
            type: "GET"
        });
    }
	...
	function get_tmp_detail(param = 0) {
		$.ajax({
			url: "<?php echo base_url() . 'pembelian/get_tr_temp_d_retur/' ?>" + btoa('edit_retur'),
			type: "GET",
			dataType: "JSON",
			success: function (data) {
    ...
    function update_tmp_detail(barcode, column, value) {
        $.ajax({
            url: "<?php echo base_url() . 'pembelian/update_tr_temp_d_retur/' ?>" + btoa(barcode) + "/" + btoa(column) + "/" + btoa(hapuskoma(value)) + "/" + btoa('edit_retur'),
            type: "GET"
        });
    }
	...
	function hapus_barang(barcode) {
		$.ajax({
			url: "<?php echo base_url() . 'pembelian/delete_tr_temp_d_retur/' ?>" + btoa(barcode) + "/" + btoa('edit_retur'),
			type: "GET",
			success: function (data) {
				if (data) {
					get_tmp_detail();
	...
	$("#barcode").keyup(function (event) {
		...
		if (event.keyCode == 13) {
			...
			if (lokasi != '' && supplier != '' && barcode != '') {
				$.ajax({
					url: "<?php echo base_url() . 'pembelian/get_barang_retur/' ?>" + btoa(nota_sistem) + "/" + btoa(barcode) + "/" + btoa(lokasi) + "/" + btoa(supplier) + "/" + btoa(cat_cari) + "/" + btoa('edit_retur'),
					type: "GET",
					dataType: "JSON",
					success: function (data) {
						if (data.status == 1) {
	...
	function batal_transaksi() {
        $.ajax({
            url: "<?php echo base_url().'pembelian/delete_trans_retur/' ?>" + btoa('edit_retur'),
            type: "GET",
            success: function (data) {
                if (data) {
                    window.location = "<?php echo base_url().'pembelian/return_pembelian_report' ?>";
                }
            }
        });
    }
