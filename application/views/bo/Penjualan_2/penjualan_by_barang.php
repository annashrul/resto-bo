							...
							<div class="row">
								<div class="col-md-12 col-sm-12 col-xs-12">
									<table class="table table-striped table-bordered">
										...
										<?php $no = 0 + (($this->uri->segment(4)!=null)?(($this->uri->segment(4)-1) * 30):0); $qt = 0; $di = 0; $gs = 0; $ns = 0; foreach($report as $row){ $no++; ?>
											<tr>
												...
												<td class="text-center">
													...
													<a href="<?=base_url().strtolower($this->control).'/'.$page.'/download/'.base64_encode($row['kd_brg'])?>" class="btn btn-icon waves-effect waves-light btn-primary btn-xs m-b-5" title="Download"><i class="md md-get-app"></i></a>
													<a href="<?=base_url().strtolower($this->control).'/'.$page.'/print/'.base64_encode($row['kd_brg'])?>" class="btn btn-icon waves-effect waves-light btn-primary btn-xs m-b-5" target="_blank" title="to PDF"><i class="md md-print"></i></a>
												</td>
											</tr>
										<?php
										$qt = $qt + (int)$row['qty'];
										...
										<tfoot>
										<tr>
											<th colspan="5">TOTAL PER PAGE</th>
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-sm-8">
							<div class="col-sm-4"><b>Kode Barang</b></div><div class="col-sm-8"><b> : </b><b id="kd_brg<?=$i?>"><?=$row['kd_brg']?></b></div>
							<div class="col-sm-4"><b>Barcode</b></div><div class="col-sm-8"><b> : </b><b><?=$row['barcode']?></b></div>
							<div class="col-sm-4"><b>Nama Barang</b></div><div class="col-sm-8"><b> : </b><b><?=$row['nm_brg']?></b></div>
							<div class="col-sm-4"><b><?=$menu_group['as_deskripsi']?></b></div><div class="col-sm-8"><b> : </b><b><?=$row['Deskripsi']?></b></div>
						</div>
						<div class="col-sm-4">
							<div class="row">
								<b><?=$periode?><b>
					...
					<div class="row">
						<div class="col-md-12 col-sm-12 col-xs-12">
							<table class="table table-striped table-bordered">
								...
								$where = "mt.HR='S' AND mt.kd_trx=dt.kd_trx AND dt.kd_brg='".$row['kd_brg']."' AND ".$tgl;
								($lokasi==null)?null:$where.=" AND mt.Lokasi='".$lokasi."'";

								$detail = $this->m_crud->read_data("Master_Trx mt, Det_Trx dt","mt.kd_trx, mt.tgl, SUM(dt.qty) qty, SUM(dt.qty*dt.hrg_jual) gross_sales, SUM(dt.dis_persen) diskon",$where,"mt.kd_trx DESC","mt.kd_trx, mt.tgl");
