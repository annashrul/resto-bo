                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-2">
                                    <div class="form-group">
                                        ...
                                        $option = null;
                                        $option[''] = 'Urutkan';
                                        $option['qty_desc'] = 'Qty Terbesar';
                                        $option['qty_asc'] = 'Qty Terkecil';
                                        $option['om_desc'] = 'Penjualan Terbesar';
                                        $option['om_asc'] = 'Penjualan Terkecil';
                                        echo form_dropdown($field, $option, isset($this->session->search[$field])?$this->session->search[$field]:set_value($field), array('class' => 'select2', 'id'=>$field));
                                        ?>
                                    </div>
                                </div>
                                        ...
                                        foreach($report as $row){ $no++; ?>
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <a href="#!" class="btn btn-icon waves-effect waves-light btn-primary btn-xs m-b-5"  title="Detail" onclick="detail('<?=$row["kd_brg"]?>','<?=$row["barcode"]?>','<?=$row["nm_brg"]?>')"><i class="md md-visibility"></i></a>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myLargeModalLabel">Detail Penjualan By Barang</h4>
            </div>
            <div class="modal-body">
                <div class="row" id="result_head"></div>
                <hr/>
                <div class="row">
...
<script>
	function detail(kode,barcode,nm_brg){
		var tgl=$("#field-date").val();
		var lok=$("#lokasi").val();
		$.ajax({
			url : "<?=base_url().'penjualan/detail_penjualan_by_barang'?>",
			type : "POST",
			dataType : "JSON",
			data:{kd_brg:kode,barcode:barcode,nm_brg:nm_brg,tanggal:tgl,lokasi:lok},
