								...
								<div class="col-md-12">
									<div class="panel-body">
										<div class="row">
											...
											<div class="col-sm-6">
												<div class="row" style="margin-bottom: 3px">
													<label class="col-sm-4">Nota Penjualan</label>
													<div class="col-sm-6">
														<div class="input-group">
															...
															*/ ?>
															<input type="text" id="<?=$field?>" name="<?=$field?>" class="form-control autocomplete_data" value="<?=set_value($field)?set_value($field):(isset($data_penjualan[0]['kd_trx'])?$data_penjualan[0]['kd_trx']:null)?>" required>
												...
												<div class="row" style="margin-bottom: 3px">
													...
													<div class="col-sm-6">
														<?php $field = 'customer' ?>
														<input type="text" id="<?=$field?>" name="<?=$field?>" class="form-control" readonly value="<?=isset($data_penjualan[0]['Nama'])?$data_penjualan[0]['Nama']:(set_value($field)?set_value($field):null)?>">
												...
												<div class="row" style="margin-bottom: 3px">
													...
													<div class="col-sm-6">
														<div class="input-group date">
															...
															<input class="form-control pull-right" readonly name="<?=$field?>" id="<?=$field?>" type="text" value="<?=set_value($field)?set_value($field):(isset($data_penjualan[0][$field])?substr($data_penjualan[0][$field],0,10):null)?>">
														</div>
													</div>
												</div>
												<div class="row" style="margin-bottom: 3px">
								...
								<div class="col-md-12 col-sm-12 col-xs-12">
									<div class="table-responsive">
										<table class="table table-striped table-bordered">
											<thead>
												<tr>
													<th style="width: 10px">No</th><th>Nota Pembelian</th><th>piutang</th><th>Retur</th><th>Jumlah piutang</th>
												</tr>
											</thead>
											<tbody id="list_barang">
												<?php $no = 0; foreach($data_penjualan as $row){ $no++; ?>
												<?php $piutang = ($row['omset']-$row['diskon_item']-$row['dis_rp']-$row['kas_lain']) - ($row['bayar'] + $row['jml_kartu'] + $row['voucher']); ?>
												<?php $retur = 0; ?>
												<?php $bayar = $row['jumlah_bayar']; ?>
												<?php $jumlah_piutang = $piutang - $retur - $bayar; ?>
												<tr>
													<td><?=$no?></td>
													<td><?=$row['kd_trx']?></td>
													<td><?=number_format($piutang, 2)?></td>
													<td><?=number_format($retur, 2)?></td>
													<td><?=number_format($jumlah_piutang, 2)?></td>
												</tr>
												<?php } ?>
											</tbody>
										</table>
								...
								<div class="col-md-12">
									<div class="panel-body">
										<div class="row">
											<div class="col-sm-6">
												<div class="row" style="margin-bottom: 3px">
													<label class="col-sm-4">Piutang</label>
													<div class="col-sm-6">
														<?php $field = 'piutang'; ?>
														<input type="text" id="<?=$field?>" name="<?=$field?>" class="form-control" readonly value="<?=isset($piutang)?number_format($piutang, 0):null?>">
													</div>
												</div>
												<div class="row" style="margin-bottom: 3px">
												...
												<div class="row" style="margin-bottom: 3px">
													...
													<div class="col-sm-6">
														<?php $field = 'bayar'; ?>
														<input type="text" id="<?=$field?>" name="<?=$field?>" class="form-control" readonly value="<?=isset($bayar)?number_format($bayar, 0):null?>">
													</div>
												</div>
												<div class="row" style="margin-bottom: 3px">
													<label class="col-sm-4">Jumlah piutang</label>
													<div class="col-sm-6">
														<?php $field = 'jumlah_piutang'; ?>
														<input type="text" id="<?=$field?>" name="<?=$field?>" class="form-control" readonly value="<?=isset($piutang)&&isset($retur)?number_format($jumlah_piutang, 0):null?>">
													</div>
												</div>
											</div>
											<div class="col-sm-6">
												<div class="row" style="margin-bottom: 3px">
													...
													<div class="col-sm-6">
														...
														<input type="text" id="<?=$field?>" name="<?=$field?>" onkeyup="hitung_pembulatan()" class="form-control text-right angka_nominal" value="<?=set_value($field)?set_value($field):0?>" />
...
$(function(){
	$('.autocomplete_data').autocomplete({
		serviceUrl: site+'penjualan/search_nota_jual'
	});	
});
...
function trx_number() {
	...
	if (tanggal != '' && lokasi != ''){
		$.ajax({
			url: "<?php echo base_url().'site/max_kode/' ?>" + btoa("BP") + "/" + btoa(tanggal) + "/" + btoa(lokasi),
			type: "GET",
...
function hitung_pembulatan(){
	var jumlah_piutang = $("#jumlah_piutang").val(); if(jumlah_piutang==''){ jumlah_piutang = 0; } jumlah_piutang = parseFloat(hapuskoma(jumlah_piutang));
	var jumlah_bayar = $("#jumlah_bayar").val(); if(jumlah_bayar==''){ jumlah_bayar = 0; } jumlah_bayar = parseFloat(hapuskoma(jumlah_bayar));
	var bulat = jumlah_bayar - jumlah_piutang;
	
	if ($("#cek_pembulatan").is(":checked") || jumlah_bayar > jumlah_piutang) {
	...
	if(jumlah_bayar > jumlah_piutang){ $("#alr_jumlah_bayar").text("Jumlah Pembayaran melebihi Jumlah Piutang! Transaksi tetap bisa dilanjut."); } else { $("#alr_jumlah_bayar").text(""); }
} 
