								...
								<div class="row">
									...
									<div class="col-sm-3">
										<div class="form-group">
											<label>Search</label>
											<?php $field = 'any'; ?>
											<input class="form-control" type="text" id="<?=$field?>" name="<?=$field?>" placeholder="Nama Member/Kode Pembayaran" value="<?=isset($this->session->search[$field])?$this->session->search[$field]:set_value($field)?>" autofocus />
										...
										<tbody>
											...
											foreach($report as $row){
                                                ...
                                                if ($row['status'] == 0) {
                                                    ...
                                                    $action = '
                                                        <li><a href="javascript:" onclick="confirm_deposit(\''.$row['member'].'\', \''.$row['id_deposit'].'\')"><i class="md md-check"></i> Confirm</a></li>
                                                        <li><a href="javascript:" onclick="cancel_deposit(\''.$row['member'].'\', \''.$row['id_deposit'].'\')"><i class="md md-close"></i> Reject</a></li>
                                                    ';
                                                    ...
                                                    $status = '<div class="panel panel-info" style="margin-bottom: -1px"><div class="panel-heading text-center" style="color: white">Waiting Approve</div></div>';
                                                    $action = '
                                                        <li><a href="javascript:" onclick="bukti_tf(\''.base_url().$row['bukti_transfer'].'\', \''.($row['nominal']+$row['kode_unik']).'\', \''.base64_encode($row['det_bank1']).'\')"><i class="md md-image"></i> Bukti Transfer</a></li>
                                                        <li><a href="javascript:" onclick="confirm_deposit(\''.$row['member'].'\', \''.$row['id_deposit'].'\')"><i class="md md-check"></i> Confirm</a></li>
                                                        <li><a href="javascript:" onclick="cancel_deposit(\''.$row['member'].'\', \''.$row['id_deposit'].'\')"><i class="md md-close"></i> Reject</a></li>
                                                    ';
                                                } else {
												...
												$decode_bank1 = json_decode($row['det_bank1'], true);
												$bank1 = 'Bank '.$decode_bank1['bank'].' ('.$decode_bank1['no_rek'].') a.n. '.$decode_bank1['atas_nama'];
                                                $decode_bank2 = json_decode($row['det_bank2'], true);
												$bank2 = 'Bank '.$decode_bank2['bank'].' ('.$decode_bank2['no_rek'].') a.n. '.$decode_bank2['atas_nama'];
												?>
												<tr>
													<td><?=$no?></td>
													...
													<td><?=substr($row['tgl_deposit'],0,19)?></td>
													<td><?=$row['id_deposit']?></td>
													<td><?=$row['nama_member']?></td>
													<td><?=number_format($row['nominal'])?></td>
													<td><?=number_format($row['kode_unik'])?></td>
													<td><?=number_format($row['nominal']+$row['kode_unik'])?></td>
													<td><?=$bank1?></td>
													<td><?=$bank2?></td>
													<td><?=$status?>
													</td>
