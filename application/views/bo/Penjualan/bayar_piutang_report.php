					...
					<div class="panel panel-default">
						<div class="panel-heading">
							...
							<div class="row">
                                ...
                                <div class="col-sm-1">
                                    <label>&nbsp;</label>
                                    <button class="btn btn-primary waves-effect waves-light" onclick="return false" id="kartu_piutang" name="kartu_piutang">Kartu Piutang</button>
                                </div>
							</div>
							<?=form_close()?>
						</div>
													...
													<td class="text-center">
														<div class="btn-group">
															...
															<li><a href="<?/*=base_url().strtolower($this->control).'/'.$page.'/print/'.base64_encode($row['no_faktur_beli'])*/?>" target="_blank"><i class="md md-print"></i> to PDF</a></li>-->
																<li><a href="<?=base_url().'pembelian/bayar_hutang/edit/'.base64_encode($row['no_nota'])?>" target="_blank"><i class="md md-edit"></i> Edit</a></li>
																<li><a href="#" id="delete" onclick="delete_trans('<?=$row['no_nota']?>')"><i class="md md-close"></i> Delete</a></li>
																<li><a href="<?=base_url().'pembelian/bayar_hutang_report/print/'.base64_encode($row['no_nota'])?>" target="_blank"><i class="md md-print"></i> to PDF</a></li>
																<li><a href="#" onclick="printDiv('print_pembelian<?=$no?>')"><i class="md md-print"></i> Print</a></li>
																<!--<li class="divider"></li>-->
															</ul>
														</div>
													</td>
...
<?php $i = 0 + (($this->uri->segment(4)!=null)?(($this->uri->segment(4)-1) * 30):0); foreach($report as $row){ $i++; ?>
	<div id="print_pembelian<?=$i?>" class="hidden">
		<table width="100%" cellspacing="0" cellpadding="1" style="letter-spacing: 5px; font-family: 'Courier New'; margin-bottom: 10px; font-size: 9pt">
			<thead>
			<tr>
				<td colspan="8" class="text-center">Nota Bayar Piutang (<?=$row['kd_trx']?>)</td>
			</tr>
			</thead>
			<tbody>
			<tr>
			...
			<tr>
				<td></td>
				<td style="font-size: 10pt !important">Nota Penjualan</td>
				<td style="font-size: 10pt !important">:</td>
				<td style="font-size: 10pt !important"><?=$row['fak_jual']?></td>
			...
			<tr>
				<td></td>
				<td style="font-size: 10pt !important">Customer</td>
				<td style="font-size: 10pt !important">:</td>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col-sm-6">
							...
							<div class="col-sm-4"><b>Nota Jual</b></div><div class="col-sm-8"><b> : </b><?=$row['fak_jual']?></div>
							<div class="col-sm-4"><b>Customer</b></div><div class="col-sm-8"><b> : </b><?=$row['Nama']?></div>
...
<div id="modal-piutang" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modal-barang" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-full">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
            ...
            <div class="modal-body">
                <div class="row" style="margin-bottom: 10px">
                    <div class="col-sm-3">
                        <div class="form-group">
                            <?php $field = 'search_customer'; ?>
                            <input class="form-control" placeholder="Search Customer" type="text" id="<?=$field?>" name="<?=$field?>" onkeyup="filter_customer($(this).val())" value="<?=isset($this->session->search[$field])?$this->session->search[$field]:set_value($field)?>" autofocus />
                            <?=form_error($field, '<div class="error" style="color:red;">', '</div>')?>
                        </div>
                    </div>
                    <div class="col-sm-9">
                        <h3 class="pull-right" id="total-piutang"></h3>
                    </div>
                </div>
                <div class="row">
                    <div class="panel-group panel-group-joined" id="list-piutang">
                    </div>
                </div>
            </div>
            <div class="modal-footer">
...
<script>
    function filter_customer(customer) {
        $.ajax({
            url: "<?php echo base_url() . 'penjualan/get_kartu_piutang' ?>",
            data: {customer_: customer},
            type: "POST",
            dataType: "JSON",
            cache: false,
            beforeSend: function () {
            ...
            success: function (res) {
                $("#list-piutang").html(res.list_piutang);
                $("#total-piutang").text(res.total_piutang);
            }
        });
    }

    $("#kartu_piutang").click(function () {
        $.ajax({
            url: "<?php echo base_url() . 'penjualan/get_kartu_piutang' ?>",
            type: "POST",
            dataType: "JSON",
            ...
            success: function (res) {
                $("#modal-title").text("Kartu Piutang");
