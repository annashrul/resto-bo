							...
							<div class="row">
								...
								<div class="col-sm-3">
									<div class="form-group">
                                        ...
                                        <?php
                                        $array_lokasi = array();
                                        if (isset($this->session->search['lokasi'])) {
                                            $data_lokasi = json_decode($this->session->search['lokasi'], true);
													...
													<td class="text-center">
														<div class="btn-group">
															...
															<ul class="dropdown-menu" role="menu">
																...
																<li><a href="#" onclick="to_excel('<?=$row['Kode']?>')"><i class="md md-print"></i> to XLS</a></li>
																<!--
																<li><a href="<?=base_url().strtolower($this->control).'/'.$page.'/print/'.base64_encode($row['Kode']).'/'.base64_encode('inventory').'/'.base64_encode(json_encode($array_lokasi)).'/'.base64_encode('pdf')?>" target="_blank"><i class="md md-print"></i> Inventory PDF</a></li>
																<li><a href="<?=base_url().strtolower($this->control).'/'.$page.'/print/'.base64_encode($row['Kode']).'/'.base64_encode('inventory').'/'.base64_encode(json_encode($array_lokasi)).'/'.base64_encode('xls')?>" target="_blank"><i class="md md-print"></i> Inventory XLS</a></li>
																-->
																<li><a href="<?=base_url().strtolower($this->control).'/'.$page.'/print/'.base64_encode($row['Kode']).'/'.base64_encode('inventory_v2').'/'.base64_encode(json_encode($array_lokasi)).'/'.base64_encode('pdf')?>" target="_blank"><i class="md md-print"></i> Inventory PDF</a></li>
																<li><a href="<?=base_url().strtolower($this->control).'/'.$page.'/print/'.base64_encode($row['Kode']).'/'.base64_encode('inventory_v2').'/'.base64_encode(json_encode($array_lokasi)).'/'.base64_encode('xls')?>" target="_blank"><i class="md md-print"></i> Inventory XLS</a></li>
                    ...
                    <div class="row">
                        <div class="col-sm-4">
                            <div class="checkbox checkbox-primary">
                                <input id="all" name="all" value="1" type="checkbox" onclick="cari(document.getElementById('kd_supp').innerHTML)">
                                <label for="all">
                                    Semua Barang
                                </label>
                            </div>
                        ...
                        <div class="col-md-5">
                            <input type="text" class="form-control" id="cari" onblur="cari(document.getElementById('kd_supp').innerHTML)">
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12 col-sm-12 col-xs-12">
                        ...
                        <table class="table table-striped table-bordered table-condensed">
                            ...
                            <tr>
                                ...
                                <th>Nama Barang</th>
                                <th style="width: 50px">Stock Awal</th>
                                <th style="width: 50px">Stock Masuk</th>
                                <th style="width: 50px">Jual</th>
                                <th style="width: 50px">Retur</th>
                                <th style="width: 50px">Adjust</th>
                                <th style="width: 50px">Mutasi</th>
                                <th style="width: 50px">Stock Akhir</th>
								<th>Harga Beli</th>
                                <th>Harga Jual</th>
                                <!--<th>Diskon Item</th>-->
                                <th>Jumlah Beli</th>
...
<script>
    function inventory(kode) {
        var tgl = $("#field-date").val();
        var lokasi = $("#lokasi").val();

        window.open("<?=base_url().'penjualan/inventory_penjualan/'?>" + btoa(kode) + '/' + btoa(lokasi) + '/' + btoa(tgl));
    }
	...
	function to_pdf(kode) {
        ...
        window.open("<?=base_url().'penjualan/detil_penjualan_group1/to_pdf/'?>" + btoa(kode) + '/' + btoa(lokasi) + '/' + btoa(tgl) + '/' + btoa(all));
	}
	
	function to_excel(kode) {
        var lokasi = $("#lokasi").val();
        var tgl = $("#field-date").val();
        var all = $("#all:checked").val();

        window.open("<?=base_url().'penjualan/detil_penjualan_group1/to_excel/'?>" + btoa(kode) + '/' + btoa(lokasi) + '/' + btoa(tgl) + '/' + btoa(all));
	}
	...
	function detail_barang(kode) {
        ...
        $.ajax({
            ...
            data: {lokasi_:lokasi, tgl_periode_: tgl, kode_:kode, all_:all},
            type: "POST",
            dataType: "JSON",
            beforeSend: function () {
                $('#loading').show();
	...
	function cari(kode) {
        ...
        $.ajax({
            url: "<?php echo base_url() . 'penjualan/detil_penjualan_group1' ?>",
            data: {lokasi_:lokasi, tgl_periode_: tgl, value_:value, filter_:filter, kode_:kode, all_:all},
            type: "POST",
            dataType: "JSON",
            beforeSend: function () {
                $('#loading').show();
