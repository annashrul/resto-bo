										...
										<div class="form-group">
											...
											<div class="input-group">
												<div class="input-group-btn">
													...
													$option = null;
													$option['Nama'] = 'Nama';
													$option['Charge_Debit'] = 'Charge Debit';
													$option['Charge_Kredit'] = 'Charge Kredit';
								...
								<div class="col-md-12 col-sm-12 col-xs-12">
									<div class="">
										<table id="" class="table table-striped table-bordered">
											<thead>
												<tr>
													<th>No</th><th>Action</th><th>Nama</th><th>EDC</th><th>Charge Debit</th><th>Charge Kredit</th><th>Nomor Rekening</th><th>Atas Nama</th><th>Status</th><th>Gambar</th>
												</tr>
											</thead>
											<tbody>
													...
													<td>
														<div class="btn-group">
															...
															<ul class="dropdown-menu" role="menu">
																<li><div class="col-sm-12"><?=anchor($content.'/edit/?trx='.base64_encode($row['Nama']), '<i class="fa fa-edit"></i> Edit', array('class'=>'btn btn-default col-sm-12'))?></div></li>
																<!--<li class="divider"></li>-->
																<!--<li><div class="col-sm-12"><?=anchor($content.'/delete/?trx='.$row['Nama'], '<i class="fa fa-trash"></i> Delete', array('class'=>'btn btn-default col-sm-12', 'onclick'=>"return confirm('Delete Data?');"))?></div></li>-->
																<li><div class="col-sm-12"><button class="btn btn-default col-sm-12" onclick="hapus('<?=$table?>', '<?='Nama'?>', '<?=base64_encode($row['Nama'])?>')"><i class="fa fa-trash"></i> Delete</button></div></li>
															</ul>
														</div>
													</td>
													<td><?=$row['Nama']?></td>
													<td><img width="25px" src="<?=base_url().'assets/images/status-'.($row['EDC']==1?'Y':'T').'.png'?>" /></td>
													<td><?=$row['Charge_Debit']+0?> %</td>
													<td><?=$row['Charge_Kredit']+0?> %</td>
													<td><?=$row['norek']?></td>
													<td><?=$row['atas_nama']?></td>
													<td><img width="25px" src="<?=base_url().'assets/images/status-'.($row['status']==1?'Y':'T').'.png'?>" /></td>
                                                    <td><img src="<?=base_url().$row['foto']?>" width="100"></td>
												</tr>
											<?php } ?>
											</tbody>
										</table>
