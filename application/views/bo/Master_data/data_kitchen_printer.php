										...
										<div class="form-group">
											...
											<div class="input-group">
												<div class="input-group-btn">
													...
													$option = null;
													$option['nama'] = 'Nama';
													$option['id_printer'] = 'Kode Printer';
													$option['konektor'] = 'Konektor';
													$option['ip'] = 'Alamat IP';
													$option['pid'] = 'Product ID';
													$option['vid'] = 'Vendor ID';
												...
												<?php $field = 'any'; ?>
												<input class="form-control" type="text" style="height: 40px" id="<?=$field?>" name="<?=$field?>" value="<?=isset($this->session->search[$field])?$this->session->search[$field]:set_value($field)?>" autofocus />
											</div>
								...
								<div class="col-md-12 col-sm-12 col-xs-12">
									<div class="">
										<table id="" class="table table-striped table-bordered">
											<thead>
												<tr>
													<th>No</th><th>Action</th><th>Kode</th><th>Nama</th><th>Konektor</th><th>Alamat IP</th><th>Product ID</th><th>Vendor ID</th>
												</tr>
											</thead>
											<tbody>
													...
													<td>
														<div class="btn-group">
															...
															<ul class="dropdown-menu" role="menu">
																<li><div class="col-sm-12"><?=anchor($content.'/edit/?trx='.base64_encode($row['id_printer']), '<i class="fa fa-edit"></i> Edit', array('class'=>'btn btn-default col-sm-12'))?></div></li>
																<li><div class="col-sm-12"><button class="btn btn-default col-sm-12" onclick="hapus('<?=$table?>', '<?='id_printer'?>', '<?=base64_encode($row['id_printer'])?>')"><i class="fa fa-trash"></i> Delete</button></div></li>
															</ul>
														</div>
													</td>
													<td><?=$row['id_printer']?></td>
													<td><?=$row['nama']?></td>
													<td><?=$row['konektor']?></td>
													<td><?=$row['ip']?></td>
													<td><?=$row['pid']?></td>
