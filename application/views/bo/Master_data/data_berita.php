								...
								<div class="col-md-12 col-sm-12 col-xs-12">
									<div class="table-responsive">
										<table id="" class="table table-striped table-bordered">
											<thead>
												<tr>
													<th>No</th><th>Action</th><th>Judul</th><th>Tanggal</th><th>Isi</th><th>Sumber</th>
												</tr>
											</thead>
											<tbody>
													...
													<td>
														<div class="btn-group">
															...
															<ul class="dropdown-menu" role="menu">
																<li><div class="col-sm-12"><?=anchor($content.'/edit/?trx='.base64_encode($row['id_berita']), '<i class="fa fa-edit"></i> Edit', array('class'=>'btn btn-default col-sm-12'))?></div></li>
																<!--<li class="divider"></li>-->
																<!--<li><div class="col-sm-12"><?=anchor($content.'/delete/?trx='.$row['id_berita'], '<i class="fa fa-trash"></i> Delete', array('class'=>'btn btn-default col-sm-12', 'onclick'=>"return confirm('Delete Data?');"))?></div></li>-->
																<li><div class="col-sm-12"><button class="btn btn-default col-sm-12" onclick="hapus('<?=$table?>', '<?='id_berita'?>', '<?=$row['id_berita']?>')"><i class="fa fa-trash"></i> Delete</button></div></li>
															</ul>
														</div>
													</td>
													<td><?=$row['judul']?></td>
													<td><?=substr($row['tanggal'], 0, 18)?></td>
                                                    <td><?=substr($row['deskripsi'], 0, 30).(strlen(strip_tags($row['deskripsi']))>30?'...':'')?></td>
                                                    <td><?=$row['sumber']?></td>
												</tr>
											<?php } ?>
											</tbody>
