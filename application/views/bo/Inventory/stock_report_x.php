							...
							<div class="row">
								<div class="col-md-12 col-sm-12 col-xs-12">
									<table class="table table-striped table-bordered">
										...
										<tbody>
										<?php $no = 0 + (($this->uri->segment(4)!=null)?(($this->uri->segment(4)-1) * 20):0); $staw = 0; $stma = 0; $stke = 0; $stak = 0; foreach($report as $row){
											$no++;
											?>
											<tr>
												<td><?=$no?></td>
												...
												<td><?=($row['stock_awal']+$row['stock_masuk']-$row['stock_keluar']+0)?></td>
												<td><a class="btn btn-primary" data-toggle="modal" onclick="detail_by_lokasi(['<?=$row['kd_brg']?>', '<?=$row['barcode']?>', '<?=$row['nm_brg']?>'], <?=$no?>)"><i class="md md-visibility"></i> Detail</a></td>
											</tr>
										<?php
											$staw = $staw + (int)$row['stock_awal'];
											$stma = $stma + (int)$row['stock_masuk'];
											$stke = $stke + (int)$row['stock_keluar'];
											$stak = $stak + (int)($row['stock_awal']+$row['stock_masuk']-$row['stock_keluar']);
										} ?>
										</tbody>
										<tfoot>
										<tr>
						...
						<table class="table table-striped table-bordered">
							...
							foreach($detail as $rows){ $no++; ?>
								<tr>
									...
									<td><?=(int)$rows['stock_in']?></td>
									<td><?=(int)$rows['stock_out']?></td>
