										...
										<tbody>
											<?php $no = 0; foreach($report as $rows){ $no++; ?>
												<tr>
													...
													<td><?=substr($rows['tgl_mutasi'], 0, 10)?></td>
													<td><?=$rows['Nama']?></td>
													<td><?=$rows['total_item']?></td>
													<td><?=(int)$rows['total_qty']?></td>
													<td><?=(int)$rows['total_approval']?></td>
													<td>
														<?php if($rows['total_approval'] < $rows['total_qty']){ ?><a href="<?=base_url().strtolower($this->control).'/approving_alokasi/'.base64_encode($rows['no_faktur_mutasi'])?>"><button type="button" class="btn btn-primary waves-effect waves-light m-b-5">Approval</button></a><?php } ?>
														<!--<div class="btn-group">
