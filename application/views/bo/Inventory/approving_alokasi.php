										...
										<tbody>
											<?php $no = 0; foreach($report_det as $rows){ $no++; ?>
												<tr>
													...
													<?php $field = 'total_qty'.$no; ?><input type="hidden" id="<?=$field?>" name="<?=$field?>" value="<?=$rows['total_qty']?>" />
													<?php $field = 'total_approval'.$no; ?><input type="hidden" id="<?=$field?>" name="<?=$field?>" value="<?=$rows['total_approval']?>" />
													<?php $sisa_approval = $rows['total_qty'] - $rows['total_approval']; ?>
													<td><?=$no?></td>
													<td><?=$rows['kd_brg']?></td>
													<td><?=$rows['barcode']?></td>
													<td><?=$rows['nm_brg']?></td>
													<td><?=(int) $rows['total_qty']?></td>
													<td><p id="v_qty_approval<?=$no?>"><?=(int) $rows['total_approval']?></p></td>
													<td><?php $field = 'sisa_approval'.$no; ?><input class="form-control" type="number" <?=($sisa_approval==0)?'disabled':null?> onclick="valid_qty(<?=$no?>)" onkeyup="valid_qty(<?=$no?>)" id="<?=$field?>" name="<?=$field?>" min="-<?=(int) ($rows['total_approval'])?>" max="<?=(int) ($sisa_approval)?>" value="<?=set_value($field)?set_value($field):($sisa_approval)?>" autofocus /></td>
													<td>
...
<script>
function approv(no){
	var qty = parseInt($('#total_qty' + no).val()); if(isNaN(qty)){ qty = 0; }
	var qty_approval = parseInt($('#total_approval' + no).val()); if(isNaN(qty_approval)){ qty_approval = 0; }
	var sisa_approval = parseInt($('#sisa_approval' + no).val()); if(isNaN(sisa_approval)){ sisa_approval = 0; }
	
	$.ajax({
		url: "<?=base_url().'site/approval_alokasi/'?>",
		type: "POST",
		data: {
			kd_trx_ : $('#kd_trx').val(),
		...
		success: function(){
			...
			$('#total_approval' + no).val(qty_approval + sisa_approval);
...
function valid_qty(no){
	...
	if( (qty == sisa_approval) && (sisa_approval <= (qty - qty_approval)) && (sisa_approval >= (-1 * qty_approval)) && (sisa_approval != 0)){
