													...
													<td class="text-center">
														<div class="btn-group">
															...
															<ul class="dropdown-menu" role="menu">
                                                                ...
                                                                <li><a href="#" onclick="re_print('<?=$row['no_faktur_mutasi']?>', cetak_pdf)" target="_blank"><i class="md md-print"></i> to PDF</a></li>
...
<div id="print_alokasi<?=$row['no_faktur_mutasi']?>" class="hidden print-nota">
    ...
    <table width="100%" cellspacing="0" cellpadding="1" style="letter-spacing: 5px; font-family: 'Courier New'; margin-bottom: 10px; font-size: 9pt">
        <thead>
        <tr>
            <td colspan="8" style="text-align: center;">Branch (<?=$row['no_faktur_mutasi']?>)</td>
        </tr>
        </thead>
        <tbody>
        <tr>
        ...
        <tr>
            ...
            <td>Status</td>
            <td>:</td>
            <td><?php if($row['status']=='0'){echo "Sending";}else{echo "Received";}?></td>
        </tr>
        </tbody>
    </table>
    ...
    <table width="100%" style="letter-spacing: 5px; font-family: 'Courier New'; font-size:9pt;">
        ...
        <tr>
            ...
            <td style="width: 15%; border-bottom: solid; border-width: thin; padding-left: 5pt" class="text-right">Harga Beli</td>
            <td style="width: 15%; border-bottom: solid; border-width: thin; padding-left: 5pt" class="text-right">Harga Jual</td>
            <td style="width: 15%; border-bottom: solid; border-width: thin; padding-left: 5pt" class="text-right">Total Beli</td>
        </tr>
        </thead>
        <tbody>
        <?php $no = 0; $total = 0; $total_beli = 0;
        $detail = $this->m_crud->join_data('Det_Mutasi as dm', 'br.kd_brg, br.barcode, br.nm_brg, dm.qty, dm.hrg_beli, dm.hrg_jual', 'barang as br', 'br.kd_brg = dm.kd_brg', "dm.no_faktur_mutasi = '".$row['no_faktur_mutasi']."'");
        foreach($detail as $rows){ $no++; ?>
            <tr>
                ...
                <td class="text-right"><?=number_format($rows['hrg_beli']*(int)$rows['qty'])?></td>
            </tr>
        <?php
        $total = $total + (int)$rows['qty'];
        $total_beli = $total_beli + ($rows['hrg_beli']*(int)$rows['qty']);
        } ?>
        </tbody>
        <tfoot>
        <tr>
            ...
            <td style="border-top: solid; border-width: thin"></td>
            <td style="border-top: solid; border-width: thin; text-align: right"><?=number_format($total_beli)?></td>
        </tr>
        </tfoot>
    </table>
			...
			<div class="modal-body">
				<div class="row">
					...
					<div class="col-sm-4">
						...
						<div class="col-sm-4"><b>Status</b></div><div class="col-sm-8"><b> : </b><?php if($row['status']=='0'){echo "Sending";}else{echo "Received";}?></div>
					</div>
				</div>	
				<hr/>
				<div class="row">
									...
									<div id="collapse<?=$row2['kd_packing']?>" class="panel-collapse collapse">
										<div class="panel-body">
											<div class="row">
												<div class="col-sm-6">
													...
													<div class="row">
														...
														<b class="col-sm-6">: <?=$row2['operator']?></b>
													</div>
													<div class="row">
											...
											<table class="table table-bordered">
												...
												<?php
												$no_pack = 1;
												$read_data_packing = $this->m_crud->read_data("master_packing mp, det_packing dp, Det_Mutasi dm, barang br","br.kd_brg, br.nm_brg, br.barcode, dm.qty qty_alokasi, dp.qty qty_packing, mp.status","mp.kd_packing=dp.kd_packing AND dp.kd_brg=br.kd_brg AND dp.kd_brg=dm.kd_brg AND mp.no_faktur_mutasi=dm.no_faktur_mutasi AND mp.kd_packing='".$row2['kd_packing']."'");
...
<script>
	function delete_trans(kode) {
		if (confirm('Akan menghapus data?')) {
			...
			hapus_otorisasi({param:'', kode:btoa(kode), activity:'Hapus Branch Alokasi', table:table_, condition:condition_}, delete_transaksi);
		}
	}
