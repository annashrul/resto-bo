...
class Api_pos extends CI_Controller {
    ...
    public function get_data($table, $perpage=10, $page=1) {
        ...
        if($action=='return'){
            if($get_data != null){
                $get_data = $this->m_website->tambah_data($table, $get_data, $lokasi);
    ...
    public function activity($perpage=10, $page=1) {
        $response = array();
        $where = "mt.HR = 'S' and mt.kd_trx=dt.kd_trx and mt.kd_cust=cs.kd_cust and mt.kd_kasir=ud.user_id";
        $where_kas = null;
        $param = $_POST['param'];
        $lokasi = $_POST['lokasi'];
        $kassa = $_POST['kassa'];
        ...
        if ($param == 'kas') {
            ...
            $read_data = $this->m_crud->select_limit('Master_Trx mt, Det_Trx dt, Customer cs, user_detail ud', "mt.kd_trx, mt.tgl, cs.Nama customer, ud.nama kasir, isnull(mt.dis_persen, 0) dis_persen, mt.dis_rp diskon_total, mt.bayar, mt.change, mt.jam, mt.status, mt.HR, mt.kassa, mt.ket_kas_lain, mt.Lokasi, mt.Jenis_trx, SUM(dt.qty * dt.hrg_jual) st, ISNULL(SUM(dt.dis_persen), 0) diskon, ISNULL(SUM(dt.tax), 0) tax, ISNULL(SUM(dt.service), 0) service, mt.compliment, mt.compliment_rp, mt.jml_kartu, mt.kartu, mt.jns_kartu, mt.no_kartu, mt.pemilik_kartu, mt.nominal_deposit, mt.poin_tukar, mt.nominal_poin, mt.no_po", $where, 'convert(date, tgl) desc, convert(time, jam) desc', "mt.kd_trx, mt.tgl, mt.tempo, cs.Nama, ud.nama, mt.dis_rp, mt.dis_persen, mt.tax, mt.bayar, mt.change, mt.jam, mt.status, mt.HR, mt.kassa, mt.ket_kas_lain, mt.Lokasi, mt.Jenis_trx, mt.compliment, mt.compliment_rp, mt.jml_kartu, mt.kartu, mt.jns_kartu, mt.no_kartu, mt.pemilik_kartu, mt.nominal_deposit, mt.poin_tukar, mt.nominal_poin, mt.no_po", $start, $end);
        }
