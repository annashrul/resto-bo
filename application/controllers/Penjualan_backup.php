        ...
        if(($action=='download'||$action=='print') && (isset($_GET['trx']) || $id!=null)){
            ...
            $data['report'] = $this->m_crud->select_limit_join(
                ...
                array(
					...
					array("type"=>"left","table"=>"sales sl"), "user_detail ud", "lokasi lk", 
					array('table'=>'compliment cm', 'type'=>'LEFT')
				), 
				array("cs.kd_cust=rt.kd_cust", "sl.kode=rt.kd_sales", "ud.user_id=rt.kd_kasir", "lk.kode=rt.lokasi", "cm.compliment_id=rt.compliment"), $where, 'rt.kd_trx asc', null, ($page-1)*$config['per_page']+1, ($config['per_page']*$page));
