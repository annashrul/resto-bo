...
//    }
	...
	public function stock_report($action = null, $id = null) {
        ...
        if(isset($_POST['search']) || isset($_POST['to_excel'])) {
            $this->session->set_userdata('search', array('any' => $_POST['any'], 'filter' => $_POST['filter'], 'filter2' => $_POST['filter2'], 'field-date'=>$_POST['field-date'], 'lokasi' => $_POST['lokasi'], 'supplier' => $_POST['supplier']));
        }

        $search = $this->session->search['any']; $lokasi = $this->session->search['lokasi']; $supplier = $this->session->search['supplier']; $filter = $this->session->search['filter']; $filter2 = $this->session->search['filter2'];$date = $this->session->search['field-date'];
        $explode_date = explode(' - ', $date);
        $date1 = str_replace('/','-',$explode_date[0]);
        $date2 = str_replace('/','-',$explode_date[1]);
        if (isset($date) && $date != null) {
        ...
        if(isset($search) && $search != null){ ($where==null)?null:$where.=" AND "; $where.=$filter2." like '%".$search."%'"; }
   ...
   function detail_by_transaksi($object) {
    ...
    public function stock_opname_report($action = null, $id = null){
        ...
        if(isset($_POST['search'])||isset($_POST['to_excel'])){
            $this->session->set_userdata('search', array('any' => $_POST['any'],'field-date'=>$_POST['field-date'], 'tgl_periode' => $_POST['tgl_periode'], 'lokasi' => $_POST['lokasi'], 'posting' => $_POST['posting'], 'selisih'=>$_POST['selisih'], 'group1'=>$_POST['group1']));
        }

        $search = $this->session->search['any']; $lokasi = $this->session->search['lokasi']; $tgl_periode = ($this->session->search['tgl_periode']==null)?$tgl_periode:$this->session->search['tgl_periode']; $posting = $this->session->search['posting']; $date = $this->session->search['field-date']; $selisih = $this->session->search['selisih']; $group1 = $this->session->search['group1'];
        $explode_date = explode(' - ', $date);
        $date1 = str_replace('/','-',$explode_date[0]);
        $date2 = str_replace('/','-',$explode_date[1]);
        ...
        if(isset($search)&&$search!=null){ ($where==null)?null:$where.=" and "; $where.="(op.kd_trx like '%".$search."%' or br.kd_brg like '%".$search."%' or br.barcode like '%".$search."%' or op.kd_kasir like '%".$search."%')"; }
        ...
        $data['report'] = $this->m_crud->select_limit('OPNAME op, barang br, kel_brg kb', "op.*, br.barcode, br.nm_brg, kb.nm_kel_brg", $where, 'op.tgl desc', null, ($page-1)*$config['per_page']+1, ($config['per_page']*$page));
        ...
        if(isset($_POST['to_excel'])){
            ...
            foreach($baca as $row => $value){
                ...
                $body[$row] = array(
					...
					$value['qty_fisik'], ($value['qty_fisik']-$value['stock_terakhir']), $value['hrg_beli'], ($value['hrg_beli'] * ((int)$value['qty_fisik']-(int)$value['stock_terakhir'])), 
        ...
        if ($get_barang_opname != null) {
            ...
            if ($tipe == 'excel' && $lokasi_ != null) {
                ...
                foreach ($baca as $row => $value) {
                    $i++;
                    $body[$row] = array(
                        $value['kd_brg'], $value['barcode'], $value['nm_brg'], $value['nm_kel_brg'], (int)$value['stock_sistem']
                    );
                    $stock = $stock + (int)$value['stock_sistem'];
                }
