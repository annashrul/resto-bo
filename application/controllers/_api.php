...
class Api extends CI_Controller {
    ...
    public function setSuccessTO(){
        ...
        $update=$this->m_crud->update_data("detail_to", array('status' => 'S'), "no_to='" . $_POST['kd_trx'] . "' and status='P'");
    ...
    public function list_produk($per_page=10, $page=1) {
        ...
        if ($type == 'barang') {
            ...
            if (isset($filter['cari']) && $filter['cari']!='') {
                $where .= ($where!=null?" AND ":"")."(br.barcode like '%".$filter['cari']."%' OR br.kd_brg like '%".$filter['cari']."%'  OR br.nm_brg like '%".$filter['cari']."%' OR kb.nm_kel_brg like '%".$filter['cari']."%')";
            }
            ...
            $get_data = $this->m_crud->select_limit_join("barang br", 'br.barcode, br.kd_brg, br.nm_brg, br.hrg_jual_1, br.gambar, br.kcp', "kel_brg kb", "kb.kel_brg=br.kel_brg", $where, "br.nm_brg ASC", null, ($page-1)*$per_page+1, $per_page*$page);
            if ($get_data == null) {
                $response['status'] = false;
            }
...
//        }
    ...
    public function exec_log() {
        ...
        $get_log = $this->m_crud->read_data("log_api", "*", "status='0'", null, null, 100);
//        var_dump($get_log);die();
        if ($get_log != null) {
