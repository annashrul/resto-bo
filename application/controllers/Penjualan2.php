...
//        var_dump($lok);
            ...
            foreach ($read_detail_piutang as $row2) {
                $list_detail_piutang .= '
                    <tr>
                        ...
                        <td>'.number_format($row2['total_jual']-$row2['total_bayar']).'</td>
    ...
    public function get_tr_temp_d_jual($tmp_param = null) {
        $param = base64_decode($tmp_param);
        $list_barang = '';
        if ($param == 'edit') {
            $get_data = $get_data = $this->m_crud->get_data("tr_temp_m", "m8, m9", "m7='edit' AND (m5 = '".$this->user."') AND (m1='Jual')");
            $read_data = $this->m_crud->read_data("tr_temp_d", "*", "d12='edit' AND (d10 = '".$this->user."') AND d1 = 'Jual'", "CONVERT(INTEGER, d13) ASC");
        } else {
            $get_data = $get_data = $this->m_crud->get_data("tr_temp_m", "m8, m9", "m7='add' AND (m5 = '".$this->user."') AND (m1='Jual')");
            $read_data = $this->m_crud->read_data("tr_temp_d", "*", "d12='add' AND (d10 = '".$this->user."') AND d1 = 'Jual'", "CONVERT(INTEGER, d13) ASC");
        }
    ...
    public function insert_tr_temp_d_jual($get_barang, $barcode, $param=null, $qty=1) {
    ...
    public function get_barang_jual($tmp_barcode, $tmp_lokasi_beli, $tmp_customer, $tmp_cat_cari, $tmp_param=null) {
        ...
        if ($get_tmp_data != '') {
            ...
            echo json_encode(array('status' => 1, 'barang'=>'tersedia', 'col'=>$get_tmp_data['d13']));
        }else {
            ...
            if ($get_barang != null) {
                if ($param == 'edit') {
                    $this->insert_tr_temp_d_jual($get_barang, $get_barang['barcode'], 'edit', $qty);
                } else {
                    $this->insert_tr_temp_d_jual($get_barang, $get_barang['barcode'], null, $qty);
                }
    ...
    public function trans_jual() {
        ...
        $bayar = 0; $change = 0; $kartu = '-'; $jml_kartu = 0; $tax = 0; $jns_kartu = '-'; $lama_tempo = '-'; $status = 'Lunas'; $tempo = $data['tgl_jual_'].' '.date("H:i:s");
    ...
    public function arsip_penjualan($action = null, $id = null) {
        ...
        if(($action=='download'||$action=='print') && (isset($_GET['trx']) || $id!=null)){
            ...
            $data['report'] = $this->m_crud->select_limit_join('report_trx rt', "rt.*, cs.nama nama_customer, sl.nama nama_waitres, ud.nama nama_kasir, isnull(cm.nama, '-') nama_compliment, lk.nama nama_lokasi", array("customer cs", "sales sl", "user_detail ud", "lokasi lk", array('table'=>'compliment cm', 'type'=>'LEFT')), array("cs.kd_cust=rt.kd_cust", "sl.kode=rt.kd_sales", "ud.user_id=rt.kd_kasir", "lk.kode=rt.lokasi", "cm.compliment_id=rt.compliment"), $where, 'rt.kd_trx desc', null, ($page-1)*$config['per_page']+1, ($config['per_page']*$page));
...
//			$data['tgl_akhir'] = $tgl_akhir;
//
//			$data['report'] = $this->m_crud->select_limit('Master_Trx mt, Det_Trx dt, barang br, kel_brg kb', "kb.kel_brg, kb.nm_kel_brg, SUM(dt.qty) qty_jual, SUM(dt.qty*dt.hrg_jual) gross_sales, SUM(dt.dis_persen) diskon_item, sum(dt.tax) tax, sum(dt.service) service", "mt.kd_trx=dt.kd_trx AND dt.kd_brg=br.kd_brg AND br.kel_brg=kb.kel_brg".($where==null?' ' : ' AND '.$where), 'kb.kel_brg ASC', "kb.kel_brg, kb.nm_kel_brg", ($page-1)*$config['per_page']+1, ($config['per_page']*$page));
//
//			$row = $this->m_crud->get_data('Master_Trx mt, Det_Trx dt, barang br, kel_brg kb', "SUM(dt.qty) qty_jual, SUM(dt.qty*dt.hrg_jual) gross_sales, SUM(dt.dis_persen) diskon_item, sum(dt.tax) tax, sum(dt.service) service", "mt.kd_trx=dt.kd_trx AND dt.kd_brg=br.kd_brg AND br.kel_brg=kb.kel_brg".($where==null?' ' : ' AND '.$where));
//
...
//		}
//        if(isset($_POST['to_excel'])){
//            $baca = $this->m_crud->read_data('Master_Trx mt, Det_Trx dt, barang br, kel_brg kb', "kb.kel_brg, kb.nm_kel_brg, SUM(dt.qty) qty_jual, SUM(dt.qty*dt.hrg_jual) gross_sales, SUM(dt.dis_persen) diskon_item, sum(dt.tax) tax, sum(dt.service) service", "mt.kd_trx=dt.kd_trx AND dt.kd_brg=br.kd_brg AND br.kel_brg=kb.kel_brg".($where==null?' ' : ' AND '.$where), 'kb.kel_brg ASC', "kb.kel_brg, kb.nm_kel_brg");
//            $header = array(
...
//            $config['total_rows'] = $this->m_crud->count_data_over("Master_Trx mt, Det_Trx dt, barang br, kel_brg kb", 'kb.kel_brg', "mt.kd_trx=dt.kd_trx AND dt.kd_brg=br.kd_brg AND br.kel_brg=kb.kel_brg ".($where==null?'':' AND '.$where), null, "kb.kel_brg");
    ...
    public function penjualan_by_customer($action = null, $id = null){
        ...
        if(isset($lokasi)&&$lokasi!=null){
            ...
            ($where==null)?null:$where.=" and "; $where.="rt.Lokasi in (".$lokasi.")";
        }
...
//        $data['table'] = $table;
//
//        $where = "mt.HR = 'S' AND mt.kd_trx=dt.kd_trx AND dt.kd_brg=br.kd_brg";
...
//
//        if(isset($_POST['to_excel'])){
//            $baca = $this->m_crud->read_data("Master_Trx mt, Det_Trx dt, barang br", "br.kd_brg, br.nm_brg, br.barcode, br.Deskripsi, br.satuan, SUM(dt.qty) qty_jual, SUM(dt.qty*dt.hrg_jual) gross_sales, SUM(dt.dis_persen) diskon_item, sum(dt.tax) tax, sum(dt.service) service", ($where==null)?null:$where, "br.kd_brg ASC", "br.kd_brg, br.nm_brg, br.barcode, br.Deskripsi, br.satuan");
//            $header = array(
//                'merge' 	=> array('A1:K1','A2:K2','A3:K3'),
//                'auto_size' => true,
//                'font' 		=> array(
...
//            $this->pagination->initialize($config);
//
//            $data['report'] = $this->m_crud->select_limit("Master_Trx mt, Det_Trx dt, barang br", "br.kd_brg, br.nm_brg, br.barcode, br.Deskripsi, br.satuan, SUM(dt.qty) qty_jual, SUM(dt.qty*dt.hrg_jual) gross_sales, SUM(dt.dis_persen) diskon_item, sum(dt.tax) tax, sum(dt.service) service", ($where==null)?null:$where, "br.kd_brg ASC", "br.kd_brg, br.nm_brg, br.barcode, br.Deskripsi, br.satuan", ($page-1)*$config['per_page']+1, ($config['per_page']*$page));
//
//            $row = $this->m_crud->get_data("Master_Trx mt, Det_Trx dt, barang br", "SUM(dt.qty) qty_jual, SUM(dt.qty*dt.hrg_jual) gross_sales, SUM(dt.dis_persen) diskon_item, sum(dt.tax) tax, sum(dt.service) service", ($where==null)?null:$where);
//
//            $tqt = (int)$row['qty_jual'];
//            $tgs = (float)$row['gross_sales'];
//            $tdi = (float)$row['diskon_item'];
...
//
////            $data['report_detail'] = $this->m_crud->read_data("Master_Trx mt, Det_Trx dt","mt.kd_trx, mt.tgl, SUM(dt.qty) qty, SUM(dt.qty*dt.hrg_jual) gross_sales, SUM(dt.dis_persen) diskon",$where,"mt.kd_trx DESC","mt.kd_trx, mt.tgl");
//            $data['report_detail'] = $this->m_crud->join_data("Master_Trx mt","mt.kd_trx, mt.tgl, SUM(dt.qty) qty, SUM(dt.qty*dt.hrg_jual) gross_sales, SUM(dt.dis_persen) diskon",$join,$on,$where,"mt.kd_trx DESC","mt.kd_trx, mt.tgl");
//
//            $count = count($data['report_detail']);
//
//            //$t_row = $t_row + 23;
...
//        $data['table'] = $table;
//
//        $where = "mt.HR = 'S' AND dt.qty>0 AND mt.kd_trx=dt.kd_trx AND br.kd_brg=dt.kd_brg AND br.Group2=gr.Kode";
//        $tgl = ''; $periode = 'Periode : None'; $q_lokasi = 'Semua Lokasi';
//
//        if(isset($_POST['search'])||isset($_POST['to_excel'])){
//            $this->session->set_userdata('search', array('any' => $_POST['any'], 'field-date'=>$_POST['field-date'], 'lokasi' => $_POST['lokasi']));
...
//            $page = ($id==null?1:$id);
//            $config['base_url'] = base_url().strtolower($this->control).'/'.$function.'/'.($action!=null?$action:'-').'/';
//            $config['total_rows'] = $this->m_crud->count_data_over("Master_Trx mt, Det_Trx dt, barang br, Group2 gr", 'gr.Kode', ($where==null)?null:$where, null, "gr.Kode");
//            $config['per_page'] = 30;
//            //$config['attributes'] = array('class' => ''); //attributes anchors
//            $config['first_url'] = $config['base_url'];
//            $config['num_links'] = 5;
...
//            $this->pagination->initialize($config);
//
//            $data['report'] = $this->m_crud->select_limit('Master_Trx mt, Det_Trx dt, barang br, Group2 gr', "gr.Kode, gr.Nama, SUM(dt.qty) qty_jual, SUM(dt.qty*dt.hrg_jual) gross_sales, SUM(dt.dis_persen) diskon_item, sum(dt.tax) tax, sum(dt.service) service", ($where==null?null:$where), 'gr.kode ASC', "gr.Kode, gr.Nama", ($page-1)*$config['per_page']+1, ($config['per_page']*$page));
//
//            $row = $this->m_crud->get_data("Master_Trx mt, Det_Trx dt, barang br, Group2 gr", "SUM(dt.qty) qty_jual, SUM(dt.qty*dt.hrg_jual) gross_sales, SUM(dt.dis_persen) diskon_item, sum(dt.tax) tax, sum(dt.service) service", ($where==null)?null:$where);
//
//            $tqt = (int)$row['qty_jual'];
//            $tgs = (float)$row['gross_sales'];
//            $tdi = (float)$row['diskon_item'];
...
//            $where = "br.Group2='".$id."'";
//            $where .= " AND ".$tgl;
//            $where .= ($lokasi==null)?"":$where.=" AND mt.Lokasi='".$lokasi."' and br.Group2='".$id."'";
//            echo $where; die();
//            $data['report_detail'] = $this->m_crud->read_data(
//                "Master_Trx mt, Det_Trx dt, barang br",
...
//                ($lokasi==null)?"":$where.=" AND mt.Lokasi='".$lokasi."' and br.Group2='".$id."'",
//                "mt.kd_trx DESC",
//                "mt.kd_trx, mt.tgl, br.kd_brg, br.barcode, br.nm_brg, br.Deskripsi"
//            );
            $data['report_detail'] = $this->m_crud->join_data(
                "Master_Trx mt",
                "mt.kd_trx, mt.tgl, br.kd_brg, br.barcode, br.nm_brg, br.Deskripsi, SUM(dt.qty) qty, SUM(dt.qty*dt.hrg_jual) gross_sales, SUM(dt.dis_persen) diskon",
                array(array("type"=>"left","table"=>"Det_Trx dt"),array("type"=>"left","table"=>"barang br")),
                array("mt.kd_trx=dt.kd_trx","dt.kd_brg=br.kd_brg"),
                ($where==null?null:$where.' and br.Group2="'.$id.'"'),"mt.kd_trx DESC","mt.kd_trx, mt.tgl, br.kd_brg, br.barcode, br.nm_brg, br.Deskripsi"
            );
//            $data =
            $count = count($data['report_detail']);
            //$t_row = $t_row + 23;
    ...
    public function detail_by_sales($kode, $tgl, $lokasi=null) {
        ...
        $where = "mt.HR='S' and mt.kd_sales='".$kode."'";
        $ket_lokasi = 'Semua Lokasi';
        $periode = $tgl_awal.' - '.$tgl_akhir;
        if ($lokasi!=null) {
            $lokasi = base64_decode($lokasi);
        ...
        if (isset($tgl) && $tgl != null) {
            $where .= " and convert(varchar, mt.tgl, 23) between '".$tgl_awal."' and '".$tgl_akhir."'";
        }
        ...
        $detail = $this->m_crud->join_data("master_trx mt", "mt.kd_trx, mt.tgl, b.kd_brg, b.barcode, b.nm_brg, b.Deskripsi, dt.hrg_jual, dt.dis_persen, isnull(jl.jual, 0) qty, isnull(rtn.retur, 0) qty_retur",
            array("det_trx dt", "barang b", array("type"=>"LEFT", "table"=>"(select qty jual, kd_trx, kd_brg from det_trx where qty>0) jl"), array("type"=>"LEFT", "table"=>"(select qty retur, kd_trx, kd_brg from det_trx where qty<0) rtn")),
            array("dt.kd_trx=mt.kd_trx", "b.kd_brg=dt.kd_brg", "jl.kd_trx=mt.kd_trx and jl.kd_brg=b.kd_brg", "rtn.kd_trx=mt.kd_trx and rtn.kd_brg=b.kd_brg"),
            $where
        );
	...
	public function detail_by_sales_test() {
//		ini_set('max_execution_time', 3600);
//		ini_set('memory_limit', '1024M');
//
		$kode = base64_decode($_POST['kode']);
		$tgl = base64_decode($_POST['tgl_periode']);
		$lokasi = null;
		$explode_date = explode(' - ', $tgl);
		$tgl_awal = str_replace('/','-',$explode_date[0]);
		$tgl_akhir = str_replace('/','-',$explode_date[1]);
		...
		if($kode=='19'){
			$hardcore .= "and convert(varchar, mt.tgl, 23) != '2019-06-02'";
		}
		if($kode=='24'){
			$hardcore .= "and convert(varchar, mt.tgl, 23) != '2019-06-10'";
		}
		...
		//ADE 2019-06-02
		//RUSH 2019-06-10
//		$dum_tgl = "convert(varchar, mt.tgl, 23) between '2019-06-11' and '2019-06-11' and ";
		$detail = $this->m_crud->join_data(
			"master_trx mt",
			"mt.kd_trx, mt.tgl, b.kd_brg, b.barcode, b.nm_brg, b.Deskripsi, dt.hrg_jual, dt.dis_persen, isnull(jl.jual, 0) qty, isnull(rtn.retur, 0) qty_retur",
			array("det_trx dt","barang b",
				array("table"=> "(select qty jual, kd_trx, kd_brg from det_trx where qty>0) jl","type"=>"LEFT"),
				array("table"=> "(select qty retur, kd_trx, kd_brg from det_trx where qty<0) rtn","type"=>"LEFT")
			),
			array("dt.kd_trx=mt.kd_trx", "b.kd_brg=dt.kd_brg", "jl.kd_trx=mt.kd_trx and jl.kd_brg=b.kd_brg", "rtn.kd_trx=mt.kd_trx and rtn.kd_brg=b.kd_brg"),
			$where.$hardcore,"mt.kd_trx"
//			,null,$this->input->post('limit'), $this->input->post('start')
		);
		$baca = $this->m_crud->join_data(
			"master_trx mt",
			"mt.kd_trx, mt.tgl, b.kd_brg, b.barcode, b.nm_brg, b.Deskripsi, dt.hrg_jual, dt.dis_persen, isnull(jl.jual, 0) qty, isnull(rtn.retur, 0) qty_retur",
		...
		if($detail != null) {
			...
			foreach ($detail as $rows) {
				$list .= '
					<tr>
						<td>'.$no.'</td>
						<td>' . $rows['kd_trx'] . '</td>
						<td>' . date("Y-m-d",strtotime($rows['tgl'])) . '</td>
						<td>' . $rows['kd_brg'] . '</td>
						<td>' . $rows['nm_brg'] . '</td>
						<td>' . $rows['nm_brg'] . '</td>
						<td>' . $rows['Deskripsi'] . '</td>
			...
			$status.=false;
			$pesan.="Data Sudah Habis";
		}
		$total_qty_kirim_s 		=0;
		$total_qty_retur_s 		=0;
		$total_qty_laku_s 		=0;
		$total_diskon_persen_s 	=0;
		$total_sub_total_s 		=0;
		foreach($baca as $value){
			$total_qty_kirim_s 		= $total_qty_kirim_s + ($value['qty']+0);
			$total_qty_retur_s 		= $total_qty_retur_s + ($value['qty_retur']+0);
			$total_qty_laku_s 		= $total_qty_laku_s + ($value['qty']-$value['qty_retur']+0);
			$total_diskon_persen_s 	= $total_diskon_persen_s + $value['dis_persen'];
			$total_sub_total_s 		= $total_sub_total_s + (($value['qty']-$value['qty_retur'])*$value['hrg_jual'])-$value['dis_persen'];
		}
		$col_total.='
		 <tr>
                <th colspan="7">Total</th>
                <th class="text-center">'.$total_qty_kirim_s.'</th>
                <th class="text-center">'.$total_qty_retur_s.'</th>
                <th class="text-center">'.$total_qty_laku_s.'</th>
                <th></th>
                <th class="text-right">'.number_format($total_diskon_persen_s, 2).'</th>
                <th class="text-right">'.number_format($total_sub_total_s, 2).'</th>
            </tr>
		';
		...
		echo json_encode(
			array(
				...
				"pesan" => $pesan,
				"status" => $status,
				'det' => array('kode'=>$kode, 'nama'=>$get_nama['Nama'], 'lokasi'=>$ket_lokasi, 'periode'=>$periode)
			)
		);
	}
