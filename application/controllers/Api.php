...
class Api extends CI_Controller {
    ...
    public function simpan_to() {
        ...
        if (is_array($data)) {
            ...
            $get_printer = $this->m_crud->read_data("kitchen_printer", "id_printer, konektor, vid, pid, ip");
            foreach ($get_printer as $item) {
                array_push($kitchen_print, array('server'=>$item['ip'], 'konektor'=>$item['konektor'], 'vid'=>$item['vid'], 'pid'=>$item['pid'], 'kd_trx'=>$kode_to, 'tanggal'=>$tanggal, 'waktu'=>$waktu, 'waitres'=>$waitres, 'atas_nama'=>$atas_nama, 'no_meja'=>$master['no_meja'], 'keterangan'=>$master['keterangan'], 'status'=>$pesan.' '.$item['nama'], 'list'=>array()));
                if($item['id_printer']=='KP0001'){
                    array_push($general_print, array('server'=>$item['ip'], 'konektor'=>$item['konektor'], 'vid'=>$item['vid'], 'pid'=>$item['pid'], 'kd_trx'=>$kode_to, 'tanggal'=>$tanggal, 'waktu'=>$waktu, 'waitres'=>$waitres, 'atas_nama'=>$atas_nama, 'no_meja'=>$master['no_meja'], 'keterangan'=>$master['keterangan'], 'status'=>$pesan.' '.$item['nama'], 'list'=>array()));
                }
                ...
                foreach ($detail as $key => $item) {
                    ...
                    if(count($get_paket)>0){
                        foreach ($get_paket as $kBahan => $iBahan) {
                            ...
                            // GENERAL PRINTER
                            array_push($general_print[0]['list'], array('kd_brg' => $iBahan['kd_brg'], 'nm_brg' => $iBahan['nm_brg'], 'ket'=>$item['ket'], 'qty' => $item['qty'], 'urutan'=>$item['urutan'], 'status' =>$pesan));
                        }
                ...
                foreach ($detail as $key => $item) {
                    ...
                    if(count($get_paket)>0){
                        foreach ($get_paket as $kBahan => $iBahan) {
                            ...
                            array_push($kitchen_print[$found_key]['list'], array('kd_brg' => $iBahan['kd_brg'], 'nm_brg' => $iBahan['nm_brg'], 'ket'=>$item['ket'], 'qty' => $item['qty'], 'urutan'=>$item['urutan'], 'status' => $status_print));

                            // GENERAL PRINTER
                            array_push($general_print[0]['list'], array('kd_brg' => $iBahan['kd_brg'], 'nm_brg' => $iBahan['nm_brg'], 'ket'=>$item['ket'], 'qty' => $item['qty'], 'urutan'=>$item['urutan'], 'status' =>$status_print));
                        }
            ...
            if ($this->db->trans_status() === FALSE || $cek_meja == false) {
                ...
                $response['print'] = $this->m_website->insert_log_print($kitchen_print, 'to', $lokasi);
                 $this->m_website->insert_log_print($general_print, 'to', $lokasi);
                $this->db->trans_commit();
            }
        } else {
            $response['status'] = false;
    ...
    public function cancel_to() {
        ...
        $get_printer = $this->m_crud->read_data("kitchen_printer", "id_printer, konektor, vid, pid, ip");
        foreach ($get_printer as $item) {
            array_push($kitchen_print, array('server'=>$item['ip'], 'konektor'=>$item['konektor'], 'vid'=>$item['vid'], 'pid'=>$item['pid'], 'kd_trx'=>$kd_trx, 'tanggal'=>$tanggal, 'waktu'=>$waktu, 'waitres'=>$this->m_crud->get_data("sales", "nama", "kode='".$get_master['kd_waitres']."'")['nama'], 'atas_nama'=>$get_master['atas_nama'], 'no_meja'=>$get_master['no_meja'], 'status'=>'Pesanan Batal'.' '.$item['nama'], 'list'=>array()));
            if($item['id_printer']=='KP0001'){
                array_push($general_print, array('server'=>$item['ip'], 'konektor'=>$item['konektor'], 'vid'=>$item['vid'], 'pid'=>$item['pid'], 'kd_trx'=>$kode_to, 'tanggal'=>$tanggal, 'waktu'=>$waktu, 'waitres'=>$waitres, 'atas_nama'=>$master['atas_nama'], 'no_meja'=>$master['no_meja'], 'keterangan'=>$master['keterangan'], 'status'=>'Pesanan Batal'.' '.$item['nama'], 'list'=>array()));
            }
        }
                ...
                $pesan='Pesanan Batal';
                    if(count($get_paket)>0){
                        ...
                        if ($item['kcp'] != '' || $item['kcp'] != null) {
                            $item['status_kcp'] = '0';
                            $found_key = array_search($item['kcp'], array_column($get_printer, 'id_printer'));
                            array_push($kitchen_print[$found_key]['list'], array('kd_brg' => $item['kd_brg'], 'nm_brg' => $item['nm_brg'], 'ket'=>$item['ket'], 'qty' => $item['qty'], 'urutan'=>$item['urutan'], 'status' => $pesan));
                        }
        ...
        if ($this->db->trans_status() === false) {
            ...
            $response['kd_trx'] = $kd_trx;
            $response['print'] = $this->m_website->insert_log_print($kitchen_print, 'to', $lokasi['lokasi']);
            $this->m_website->insert_log_print($general_print, 'to', $lokasi['lokasi']);
    ...
    public function doRePrint(){
        $kd_trx=$_POST['kd_trx'];
        $get_master = $this->m_crud->get_data("master_to", "*", "no_to='".$kd_trx."'");
        $get_detail = $this->m_crud->read_data("detail_to", "*", "no_to='".$kd_trx."'");
        $pesan="CETAK ULANG";
        $param_meja = $kd_trx;
        $lokasi = $this->m_crud->get_data("master_to", "lokasi", "no_to='".$kd_trx."'")['lokasi'];
        $waitres = $this->m_crud->get_data("sales", "nama", "kode='".$get_master['kd_waitres']."'")['nama'];
        $date=date_create($get_master['tanggal']);
        $time=date_create($get_master['waktu']);
        $tanggal=date_format($date,"Y/m/d");
        $waktu=date_format($time,"H:i:s");

        $kitchen_print = array();
        $general_print = array();
        $get_printer = $this->m_crud->read_data("kitchen_printer", "id_printer, konektor, vid, pid, ip");
        foreach ($get_printer as $item) {
            array_push($kitchen_print, array('server'=>$item['ip'], 'konektor'=>$item['konektor'], 'vid'=>$item['vid'], 'pid'=>$item['pid'], 'kd_trx'=>$kd_trx, 'tanggal'=>$tanggal, 'waktu'=>$waktu, 'waitres'=>$waitres, 'atas_nama'=>$get_master['atas_nama'], 'no_meja'=>$get_master['no_meja'], 'keterangan'=>$get_master['keterangan'], 'status'=>$pesan, 'list'=>array()));
            if($item['id_printer']=='KP0001'){
                array_push($general_print, array('server'=>$item['ip'], 'konektor'=>$item['konektor'], 'vid'=>$item['vid'], 'pid'=>$item['pid'], 'kd_trx'=>$kode_to, 'tanggal'=>$tanggal, 'waktu'=>$waktu, 'waitres'=>$waitres, 'atas_nama'=>$get_master['atas_nama'], 'no_meja'=>$get_master['no_meja'], 'keterangan'=>$get_master['keterangan'], 'status'=>$pesan, 'list'=>array()));
            }

        }
        
         foreach ($get_detail as $key => $item) {
            $item['status_kcp'] = '1';
            ...
            if(count($get_paket)>0){
                ...
                if ($item['kcp'] != '' || $item['kcp'] != null) {
                    $item['status_kcp'] = '0';
                    $found_key = array_search($item['kcp'], array_column($get_printer, 'id_printer'));
                    array_push($kitchen_print[$found_key]['list'], array('kd_brg' => $item['kd_brg'], 'nm_brg' => $get_barang['nm_brg'], 'ket'=>$item['ket'], 'qty' => $item['qty'], 'urutan'=>$item['urutan'], 'status' => $pesan));
                }
                array_push($general_print[0]['list'], array('kd_brg' => $item['kd_brg'], 'nm_brg' => $get_barang['nm_brg'], 'ket'=>$item['ket'], 'qty' => $item['qty'], 'urutan'=>$item['urutan'], 'status' =>$pesan));
            }

            
        }

        $this->m_website->insert_log_print($kitchen_print, 'to', $get_master['lokasi']);
        $this->m_website->insert_log_print($general_print, 'to', $get_master['lokasi']);

        echo json_encode(array("status"=>true));
    }

    public function setSuccessTO(){
        $this->m_crud->update_data("master_to", array('status' => 'S'), "no_to='" . $_POST['kd_trx'] . "'");
        $update=$this->m_crud->update_data("detail_to", array('status' => 'S'), "no_to='" . $_POST['kd_trx'] . "' and status='R'");
    ...
    public function exec_log() {
        ...
        $get_log = $this->m_crud->read_data("log_api", "*", "status='0' AND param='exec'", null, null, 100);
        ...
        if ($get_log != null) {
            foreach ($get_log as $item) {
                ...
                if ($this->db->trans_status() === false) {
                    if ($err['code'] == '23000/2627' || $err['code'] == '2627') {
    ...
    public function get_meja() {
        ...
        $get_meja = $this->m_crud->join_data(
            "data_meja m", 
            "m.*, (m.nama_area + ' ' + m.nama_meja) text, isnull(mt.no_to, '') no_to, isnull(mt.tanggal, '') tanggal, isnull(mt.waktu, '') waktu, isnull(mt.jml_tamu, '') jml_tamu, isnull(mt.atas_nama, '') atas_nama, isnull(mt.gabung, '') gabung, isnull(s.nama, '') waitres", 
            array(array('type'=>'LEFT','table'=>'master_to mt'), array('type'=>'LEFT','table'=>'sales s')),
            array("mt.meja=m.id_meja and mt.status='P'", "s.Kode=mt.kd_waitres"), 
            "m.id_area='".$area."'", "cast(m.nama_meja as int) asc");
