...
class Pembelian extends CI_Controller {
    ...
    public function pembelian_barang_report($action = null, $id = null){
        ...
        $tgl_awal = date('Y-m-d'); $tgl_akhir = date('Y-m-d');
        if(isset($_POST['search'])||isset($_POST['to_excel'])){
            $this->session->set_userdata('search', array('any' => $_POST['any'],'field-date'=> $_POST['field-date'], 'lokasi' => $_POST['lokasi'], 'tipe' => $_POST['tipe'], 'order_by'=>$_POST['order_by'], 'order_sort'=>$_POST['order_sort'], 'pelunasan'=>$_POST['pelunasan']));
        }
		...
		$pelunasan = $this->session->search['pelunasan'];
        $explode_date = explode(' - ', $date);
        $date1 = str_replace('/','-',$explode_date[0]);
        $date2 = str_replace('/','-',$explode_date[1]);
        if (isset($date) && $date != null) {
		...
		if(isset($pelunasan)&&$pelunasan!=null){
			($where==null)?null:$where.=" AND ";
			$where.="pr.Pelunasan = '".$pelunasan."'";
		}
        $page = ($id==null?1:$id);
        $config['base_url'] = base_url().strtolower($this->control).'/'.$function.'/'.($action!=null?$action:'-').'/';
